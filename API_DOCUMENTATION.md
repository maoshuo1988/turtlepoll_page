...
### 9. 获取话题列表
- **接口**: `GET /api/topic/topics`
- **描述**: 获取话题列表（支持按节点筛选，也支持个人中心业务列表）
- **认证**: 部分认证（`nodeId=-2` 关注节点时需要登录；`business_type` 为 `1`~`6` 时需要登录）
- **查询参数**:
  - `nodeId`: 节点 ID（可选，默认 `0`；支持内置节点 `0`/`-1`/`-2`）
  - `cursor`: 分页游标（可选，默认 `0`）
  - `business_type`: 业务列表类型（可选，默认 `0`）
- **business_type 取值**:
  - `0`: 默认帖子列表，保持原有逻辑
  - `1`: 登录用户的帖子列表（自己创建的帖子）
  - `2`: 已收藏的帖子列表（收藏别人的帖子）
