...
- 鉴权：部分（`nodeId=-2` 关注节点时需要登录；`business_type` 为 `1`~`6` 时需要登录）
- Query 参数：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `nodeId` | number | 否 | 节点 ID，默认 `0`；支持内置节点：`0` 默认列表，`-1` 推荐，`-2` 关注，`>0` 普通节点。 |
| `cursor` | number | 否 | 游标，默认 `0`，首屏传 `0`。 |
| `business_type` | number | 否 | 业务列表类型，默认 `0`；`0` 默认帖子列表，保持原有逻辑。 |
...
|---|---|---|
| `0` | 默认帖子列表，保持原有逻辑。 | 可选 |
| `1` | 登录用户的帖子列表（自己创建的帖子）。 | 需要登录 |
| `2` | 已收藏的帖子列表（收藏别人的帖子）。 | 需要登录 |
| `3` | 已隐藏的帖子列表（隐藏自己的帖子）。 | 需要登录 |
| `4` | 已点赞的帖子列表（用户点赞别人的帖子）。 | 需要登录 |
| `5` | 已点踩列表（用户点踩别人的帖子）。 | 需要登录 |
| `6` | 已评论列表（用户评论别人的帖子）。 | 需要登录 |
