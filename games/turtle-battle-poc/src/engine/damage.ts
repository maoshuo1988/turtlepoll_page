...
// P25b: 连笔伤害传递的可见化钩子 — BattleScene 注入 (飘字 + 战绩记录)。
//   本文件主体是纯公式; inkLink 传递本就是 P25 的副作用 (在中心扣血点重定向), 这里只让它可见。
let _inkTransferHook: ((partner: Fighter, shown: number, dmgType: 'magic' | 'true', owner: Fighter | null) => void) | null = null;
export function setInkTransferHook(fn: ((partner: Fighter, shown: number, dmgType: 'magic' | 'true', owner: Fighter | null) => void) | null): void {
  _inkTransferHook = fn;
}
...
export function calcDamage(atk: Fighter, tgt: Fighter, base: number, dmgType: DamageType): number {
  ...
  // 注: diamondStructure 减伤已移到 applyRawDamage 唯一收口处 (避免与 deal* 双减 + 让手算技能也享受)
...
export function applyRawDamage(
  ...
  /** 攻击方 fighter — 用于读 `_dmgBonusThisTurnPct` (FPGA-10 / 信号放大器 本回合 +X% 增伤, JS combat.js:888 1:1) */
  ...
  // 必中标记 markedDmg (c_mark / q_mark 🎯): 受到的所有伤害 +value% (任何来源, 含真伤)。
  //   修(2026-05-30 用户审装备): 此 buff 在原版 JS 与 PoC 都只 set 从不消费 → 必中标记两个版本都完全无效。
  //   在中心扣血点实装: 在所有减伤/吸收之前放大 incoming, 让标记真正 "受伤+20%"。
  {
    const mk = tgt.buffs.find(b => b.type === 'markedDmg');
    if (mk && (mk.value ?? 0) > 0 && finalDmg > 0) finalDmg = Math.round(finalDmg * (1 + (mk.value as number) / 100));
  }
  // FPGA-10 / 信号放大器: 攻击方本回合 +X% 增伤 (JS combat.js:888 1:1)。
  //   "增伤"是独立概念, 影响所有伤害(物理/魔法/真伤/装备 proc/DoT 都行), 不只 ATK 缩放。
  //   FPGA-10 / 放大器共用 `_dmgBonusThisTurnPct` 字段, Math.max 取较大 (在 BattleScene 设)。
  //   每回合开始 BattleScene 重置为 0。
  if (attacker && finalDmg > 0) {
    const bonus = (attacker as Fighter & { _dmgBonusThisTurnPct?: number })._dmgBonusThisTurnPct ?? 0;
    if (bonus > 0) finalDmg = Math.round(finalDmg * (1 + bonus / 100));
  }
  ...
  // 磐石之躯 岩层减伤: 每层 1% (cap 30 → 最多 30%), 真伤跳过。层数在 passive-triggers 受击累积。
  if (dmgType !== 'true' && finalDmg > 0) {
    const rl = Math.min(30, (tgt as Fighter & { _rockLayers?: number })._rockLayers ?? 0);
    if (rl > 0) finalDmg = Math.max(1, Math.round(finalDmg * (1 - rl / 100)));
  }
  // E3/10 (combat.js:182-187): diamondStructure 钻石结构 固定减伤 — 唯一收口处
  //   物理/魔法 减 (def×defPct + mr×mrPct); 强化后 def20%+mr10%, 未强化 def20%。真伤跳过。
  //   放这里: deal* 与手算技能 (iceFrost 等直接走 applyRawDamage) 都统一享受, 不再双减/漏减。
  if (dmgType !== 'true' && finalDmg > 0 && tgt.passive?.type === 'diamondStructure') {
    ...
    if (flatReduce > 0) finalDmg = Math.max(1, finalDmg - flatReduce);
  }
  ...
  if (goesThroughShield) {
    ...
    // 特殊限时护盾池: 熔岩盾(凤凰)/缩头盾 — 独立吸收, 到期清/转血 (并入 auraAbs 让 shown 统计正确)
    const ts2 = tgt as Fighter & { _lavaShieldVal?: number; _hidingShieldVal?: number };
    if ((ts2._lavaShieldVal ?? 0) > 0 && remaining > 0) {
      const a = Math.min(ts2._lavaShieldVal!, remaining); ts2._lavaShieldVal! -= a; remaining -= a; auraAbs += a;
    }
    if ((ts2._hidingShieldVal ?? 0) > 0 && remaining > 0) {
      const a = Math.min(ts2._hidingShieldVal!, remaining); ts2._hidingShieldVal! -= a; remaining -= a; auraAbs += a;
    }
  ...
  // ════════ 受伤累积型(target-side)被动 — 中心扣血点统一结算 (JS passive_subscribers.js bus 订阅) ════════
  //   JS 用中心 'damage:dealt' 总线: DoT/爆裂/反弹/真伤 一切伤害都触发。PoC 之前放在 triggerOnHitEffects
  //   (仅标准技能命中链) → DoT/爆裂等不走 on-hit 链的伤害漏积累。现搬到本唯一扣血 chokepoint, 一处覆盖。
  //   注: 仅 target-side (累积者=被打的人) 能搬这里 — applyRawDamage 不知 attacker, 故 lavaRage 攻击端 /
  //   chestTreasure 仍留 triggerOnHitEffects。视觉(面板条/徽章)走 liveSig 轮询刷新, 不在引擎层飘字。
  const acc = tgt as Fighter & {
    bubbleStore?: number; _auraEnergy?: number; _lavaRage?: number; _lavaRageReady?: boolean;
    _lavaSpent?: boolean; _lavaTransformed?: boolean; _twoHeadResilience?: boolean; _twoHeadResStacks?: number;
  };
  // 泡泡龟 bubbleStore — 用 hpLoss(打盾不积), 仅存活, cap=maxHp (JS passive_subscribers.js:90)
  if (tgt.alive && hpLoss > 0 && tgt.passive?.type === 'bubbleStore') {
    const stored = Math.round(hpLoss * ((tgt.passive.pct as number) ?? 100) / 100);
    if (stored > 0) acc.bubbleStore = Math.min(Math.round(tgt.maxHp), (acc.bubbleStore ?? 0) + stored);
  }
  // 龟壳 auraAwaken 储能 — 用 hpLoss(打盾不积), 仅存活, cap=maxHp×energyMaxStorePct (JS:77)
  if (tgt.alive && hpLoss > 0 && tgt.passive?.type === 'auraAwaken' && tgt.passive.energyStore) {
    const cap = Math.round(tgt.maxHp * ((tgt.passive.energyMaxStorePct as number) ?? 0.5));
    acc._auraEnergy = Math.min(cap, (acc._auraEnergy ?? 0) + hpLoss);
  }
  // 熔岩龟 lavaRage 受伤端怒气 — 用 hpLoss(打盾不积), 满则标记变身 (JS:62)
  if (hpLoss > 0 && tgt.passive?.type === 'lavaRage' && !acc._lavaSpent && !acc._lavaTransformed) {
    const max = (tgt.passive.rageMax as number) ?? 100;
    const next = Math.min(max, (acc._lavaRage ?? 0) + Math.round(hpLoss * ((tgt.passive.rageTakenPct as number) ?? 20) / 100));
    acc._lavaRage = next;
    if (next >= max) acc._lavaRageReady = true;
  }
  // 双头龟 twoHeadResilience 受击叠甲抗 — 每受一段攻击 +1甲+1抗 (cap 20), 用 finalDmg(攻击落地即算, 含被盾吸)
  //   (JS pets.js:251-253 / bus 订阅 amount>0; 面板 _twoHeadResStacks 徽章随 liveSig 刷新, 去掉旧 +1甲+1抗 飘字)
  if (finalDmg > 0 && acc._twoHeadResilience && (acc._twoHeadResStacks ?? 0) < 20) {
    acc._twoHeadResStacks = (acc._twoHeadResStacks ?? 0) + 1;
    tgt.baseDef += 1; tgt.def += 1;
    tgt.baseMr = (tgt.baseMr ?? tgt.def) + 1; tgt.mr += 1;
  }
  ...
  const linkRef = (tgt as Fighter & { _inkLink?: { partner: Fighter; turns: number; transferPct: number; dmgType?: 'magic' | 'true'; owner?: Fighter } })._inkLink;
  ...
  if (!_skipLink && linkRef && linkRef.partner && linkRef.partner.alive && totalShown > 0) {
    ...
    if (transferAmt > 0) {
      ...
      const tr = applyRawDamage(linkRef.partner, transferAmt, linkType, linkType === 'true', true);
      // P25b: 让传递可见 — 飘字 + 战绩 (之前传递只扣血, 无飘字无统计 → 看着像"没传")
      const tShown = (tr.hpLoss ?? 0) + (tr.shieldAbs ?? 0);
      _inkTransferHook?.(linkRef.partner, tShown, linkType, linkRef.owner ?? null);
    }
  }
