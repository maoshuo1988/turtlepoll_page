...
export function createFighter(
  ...
  // 磐石之躯(rockShockwave) 岩层被动门控: 仅装备该技能时, 受击叠岩层 (passive-triggers + applyRawDamage 读 _rockLayers).
  const fx = f as Fighter & { _hasRockArmor?: boolean; _rockLayers?: number };
  fx._hasRockArmor = skills.some(s => s.type === 'rockShockwave');
  fx._rockLayers = 0;
