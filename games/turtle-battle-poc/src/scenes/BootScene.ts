...
export class BootScene extends Phaser.Scene {
  ...
  /** DOM warmup throw-away texture keys — load.complete 时统一 evict 释放 CPU 内存。 */
  private domWarmupKeys: string[] = [];
  ...
  preload() {
    ...
    this.load.on('complete', () => {
      // 把 DOM warmup throw-away texture 从 Phaser TextureManager 里清掉, 释放 CPU 内存。
      // HTTP cache 不丢, DOM <img> 后续仍 cache hit。
      this.evictDomWarmupTextures();
    ...
    const ACTION_SHEETS: Array<{ pet: string; action: string; file: string; w: number; h: number }> = [
      ...
      // 中立宝箱怪 (treasure_golem) 全套动作: attack 5帧 / hurt 4帧 / death 5帧 / knockup 2帧, 74×74
      //   素材 2026-05-30 用户提供, 原放 games/根 中文名, 已归位 games/turtle-battle/assets/pets/animations/treasure_golem/
      { pet: 'treasure_golem', action: 'attack',  file: 'attack.png',  w: 74, h: 74 },
      { pet: 'treasure_golem', action: 'hurt',    file: 'hurt.png',    w: 74, h: 74 },
      { pet: 'treasure_golem', action: 'death',   file: 'death.png',   w: 74, h: 74 },
      { pet: 'treasure_golem', action: 'knockup', file: 'knockup.png', w: 74, h: 74 },
    ];
    ...
    // 宝箱龟「专属装备」三池 icon — 单独键 `chest-equip-${id}`, 让 turtle-hud 在角色身侧渲染
    //   (chest 池物品 id 没 e_ 前缀, 跟 EQUIP_POOL 不冲突; 图标也是另一套 equip-X-icon.png / passive-...)
    const chestPet = ALL_PETS.find(p => p.id === 'chest');
    const chestPools = (chestPet?.passive as { pools?: Array<Array<{ id?: string; icon?: string }>> } | undefined)?.pools;
    if (chestPools) {
      const loaded = new Set<string>();
      for (const pool of chestPools) {
        for (const eq of pool) {
          if (eq.id && eq.icon && eq.icon.endsWith('.png') && !loaded.has(eq.id)) {
            loaded.add(eq.id);
            this.load.image(`chest-equip-${eq.id}`, eq.icon.replace(/\/([^/]+\.png)$/, '/_sm/$1'));
          }
        }
      }
    }

    // ═══ DOM <img> HTTP cache warmup (2026-05-30 用户: "把要加载的都集中在预加载") ═══
    // 战斗内的 DOM <img src=...> 用浏览器原生 fetch — 但 URL 跟 Phaser preload 不同
    //   (DOM 用"原图" e.g. equip/dungeon-fpga.png 1254px; Phaser 用 _sm/dungeon-fpga.png 128px)
    // → 浏览器 HTTP cache 没命中 → 战斗中现拉 → 卡。
    // 此处用 this.load.image() 把所有 DOM <img> 会用的 URL 走一遍, HTTP cache 填满后,
    // DOM <img> 全部 cache hit, 战斗中零网络。
    // 走完后 evictDomWarmupTextures() 移除 throw-away texture 释放 CPU 内存 (HTTP cache 不丢)。
    this.setupDomWarmup();

    // Audio
    this.load.audio('bgm-menu', `audio/bgm-menu.mp3`);
    this.load.audio('bgm-battle', `audio/bgm-battle.mp3`);
    this.load.audio('bgm-boss', `audio/bgm-boss.mp3`);   // I5: boss 关 BGM
  ...
  /** 注册一次 DOM warmup — 走 Phaser load.image 让 HTTP cache 填上。
   *  用 throw-away key 避免和真 texture 冲突; 同 URL 去重防重复请求。 */
  private warmDom(path: string, seen: Set<string>): void {
    if (!path || seen.has(path)) return;
    seen.add(path);
    const key = `dom-warm-${this.domWarmupKeys.length}`;
    this.domWarmupKeys.push(key);
    this.load.image(key, path);
  }

  /** 枚举所有战斗内 DOM <img> 会用的 URL → load.image 一遍。 */
  private setupDomWarmup(): void {
    const seen = new Set<string>();

    // 1. 装备原图 (~24 MB, 1254×1254) — DetailPanel/turtle-hud/ShopOverlay 各处 DOM <img src=eq.icon>
    for (const eq of EQUIP_POOL) {
      if (eq.icon?.endsWith('.png')) this.warmDom(eq.icon, seen);
    }
    // 1b. 宝箱龟专属池 icon (跟 EQUIP_POOL 不重叠)
    const chestPet = ALL_PETS.find(p => p.id === 'chest');
    const chestPools = (chestPet?.passive as { pools?: Array<Array<{ icon?: string }>> } | undefined)?.pools;
    if (chestPools) {
      for (const pool of chestPools) for (const eq of pool) {
        if (eq.icon?.endsWith('.png')) this.warmDom(eq.icon, seen);
      }
    }

    // 2. 状态原图 (非 _sm) — DetailPanel rock badges / 详情 hover
    const STATUS_FULL = ['burn', 'poison', 'bleed', 'chilled', 'curse-debuff', 'dodge',
      'fear', 'heal-reduce', 'reflect', 'shield', 'stealth', 'stun', 'taunt', 'dodge-new'];
    for (const s of STATUS_FULL) this.warmDom(`status/${s}-icon.png`, seen);

    // 3. 被动原图 (非 _sm) — DetailPanel / 飘字 / hover
    const PASSIVE_FULL = ['aura-awaken', 'bamboo-charge', 'bubble-store', 'candy-steal',
      'chest-treasure', 'crystal-resonance', 'cyber-drone', 'diamond-structure',
      'fortune-gold', 'frost-aura', 'ghost-curse', 'gambler-blood', 'gambler-multi',
      'hunter-kill', 'judgement', 'lava-heart', 'lightning-storm', 'mech-form',
      'ninja-instinct', 'phoenix-rebirth', 'pirate-plunder', 'rainbow-prism',
      'star-energy', 'stone-wall', 'summon-ally', 'two-head', 'undead-rage',
      'unyielding', 'volcano-form'];
    for (const p of PASSIVE_FULL) this.warmDom(`passive/${p}-icon.png`, seen);

    // 4. 技能图 (~52 MB) — TeamSelectScene / DetailPanel 技能行
    for (const pet of ALL_PETS) {
      const pool = (pet as { skillPool?: Array<{ icon?: string }> }).skillPool ?? [];
      for (const sk of pool) {
        if (sk.icon?.endsWith('.png')) this.warmDom(sk.icon, seen);
      }
    }
    // 4b. DetailPanel rock badges 里的 literal 技能图引用
    for (const lit of ['skills/lightning-4.png', 'skills/twohead-resilience.png', 'skills/diamond-collide.png']) {
      this.warmDom(lit, seen);
    }

    // 5. 羁绊标签 — DetailPanel synergy + TeamSelect synergy chip
    for (const tag of Object.keys(SYNERGY_TAGS)) {
      this.warmDom(`tags/${tag}标签.png`, seen);
    }

    // 6. 属性图标 — DetailPanel / TeamSelect 属性面板
    for (const s of ['hp', 'atk', 'def', 'mr']) this.warmDom(`stats/${s}-icon.png`, seen);
    this.warmDom('stats/rock-layer-icon.png', seen);

    // 7. 特殊头像
    //   - 机甲: DetailPanel 改用 pets/mech.png (BootScene line 82 已 preload, 不需 warmup)
    //   - 水晶球召唤: SUMMON_AVATAR['crystal-ball'] = 'skills/crystal-3.png' (DOM 用)
    this.warmDom('skills/crystal-3.png', seen);

    // 8. 战斗 UI
    this.warmDom('battle/deep-coin.png', seen);

    // 9. UI 按钮 (战斗顶栏 + GlobalToolbar)
    for (const b of ['back', 'help', 'log', 'stats', 'sound', 'fullscreen']) {
      this.warmDom(`ui/btn-${b}.png`, seen);
    }
  }

  /** load.complete 后清理 throw-away texture (HTTP cache 不丢)。 */
  private evictDomWarmupTextures(): void {
    for (const k of this.domWarmupKeys) {
  ...
  create() {
    ...
    // 动作动画 (单次播放) —— basic/ghost/ninja/treasure_golem 全套 attack/hurt/death/knockup
    const ACTION_PETS = ['basic', 'ghost', 'ninja', 'treasure_golem'];
  ...
  private makeShadowTexture() {
    ...
    // 贴图烤成满黑 (中心 1.0), 实际深浅由 shadow.setAlpha() 控制 → 调试器"深浅"滑块全范围可调
    //   (0.05 很淡 → 1.0 很深)。实色区铺到 ~60% 再柔化到边缘, 保留柔边 + 透草感。
    const grad = ctx.createRadialGradient(gx, gy, 0, gx, gy, r);
    grad.addColorStop(0,    'rgba(0,0,0,1.00)');
    grad.addColorStop(0.60, 'rgba(0,0,0,0.85)');
    grad.addColorStop(0.85, 'rgba(0,0,0,0.30)');
