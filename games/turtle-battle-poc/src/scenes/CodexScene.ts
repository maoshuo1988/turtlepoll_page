...
export class CodexScene extends Phaser.Scene {
  ...
  private showPetDetail(pet: PetDef, view: 'skill-list' | 'form-list' | { skillIdx: number; form?: boolean } | 'passive' = 'skill-list') {
    ...
    // E1: 双形态龟 — 火山(volcanoSkills, 熔岩龟变身) 或 近战(meleeSkills, 双头龟换形) 都提供"形态切换"按钮。
    const petForm = pet as PetDef & { volcanoSkills?: PetDef['skillPool']; meleeSkills?: PetDef['skillPool'] };
    const isMeleeForm = Array.isArray(petForm.meleeSkills) && petForm.meleeSkills.length > 0;
    const formSkills = isMeleeForm ? petForm.meleeSkills : petForm.volcanoSkills;
    ...
    if (hasForm && (view === 'skill-list' || view === 'form-list')) {
      const label = isMeleeForm
        ? (isFormView ? '🏹 查看 远程形态技能' : '⚔️ 查看 近战形态技能')
        : (isFormView ? '🐢 查看 普通形态技能' : '🌋 查看 火山形态技能');
