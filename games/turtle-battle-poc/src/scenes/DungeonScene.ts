...
export class DungeonScene extends Phaser.Scene {
  ...
  // #7 整局规则: 第 1 关随机定一条 (排除正常对局), 整个深海闯关沿用同一规则
  private static _cachedRule: string | null = null;
  ...
  init(data: DungeonInit) {
    ...
    // #7 整局规则: 第 1 关随机抽一条非「正常对局」的规则, 缓存供后续关沿用 (整局统一)
    if (this.stage <= 1 || DungeonScene._cachedRule == null) {
      const pool = BATTLE_RULES.filter(r => r.id !== 'normal');
      DungeonScene._cachedRule = pool[Math.floor(Math.random() * pool.length)].name;
    }
    this.rule = DungeonScene._cachedRule;
  }
  ...
  create() {
    ...
    // #7 整局规则 chip — 显示本局深海规则 (全程同一条)
    const rd = BATTLE_RULES.find(r => r.name === this.rule);
    if (rd && rd.id !== 'normal') {
      const colorHex = '#' + rd.color.toString(16).padStart(6, '0');
      this.add.text(width / 2, 238, `${rd.emoji} ${rd.name} — ${rd.desc}`, {
