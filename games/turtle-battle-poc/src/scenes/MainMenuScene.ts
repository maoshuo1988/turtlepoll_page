...
export class MainMenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 在线模式: 快速匹配 / 房间对战 (2026-05-30: 未实装, disabled + 「敬请期待」toast) + 返回
    this.onlineGroup = this.add.container(0, 0).setDepth(4);
    onlineBtns.push(mkBtn(this.onlineGroup, 0, '快速匹配', () => this.showComingSoonToast(), true));
    onlineBtns.push(mkBtn(this.onlineGroup, 1, '房间对战', () => this.showComingSoonToast(), true));
    ...
    // 自定义子组: 野生对局 / 指定Boss [+ DEV: 测试模式 / 快速单体调试] + 返回(回本地)
    // DEV gate (2026-05-30): 「测试模式」「快速单体调试」仅 ?dev=1 显示, slot 自动顺移。
    this.customGroup = this.add.container(0, 0).setDepth(4);
    let slot = 0;
    customBtns.push(mkBtn(this.customGroup, slot++, '野生对局', () => this.openTeamSelect('pve')));
    customBtns.push(mkBtn(this.customGroup, slot++, '指定 Boss', () => this.openTeamSelect('boss-pick')));
    if (DEV_VISIBLE) {
      customBtns.push(mkBtn(this.customGroup, slot++, '测试模式', () => this.openTeamSelect('test')));
      customBtns.push(mkBtn(this.customGroup, slot++, '快速单体调试', () => this.openSoloDebug()));
    }
    customBtns.push(mkBtn(this.customGroup, slot++, '← 返回', () => this.goToScreen('local')));
  ...
  private makeFrameButtonInto(parent: Phaser.GameObjects.Container, x: number, y: number, label: string, onClick: () => void, disabled = false, w = 360, h = 85) {
    ...
    if (disabled) {
      bg.setTint(0x888888);
      // 修(2026-05-30): 之前 disabled 直接 return → onClick 被丢, 灰按钮点了无任何反馈。
      //   现接 pointerdown 触发 cb (online 按钮借此显「敬请期待」toast)。
  ...
  /** 快速单体调试: DOM 浮层选 1 只龟 + 勾技能(5选3, 调试用无视等级锁) → 进 test 战斗 (单龟 vs 6 假人) */
  private openSoloDebug() {
    ...
    ov.innerHTML = `
      <div style="background:#13203a;border:2px solid #6fa8ff;border-radius:14px;padding:22px 26px;width:min(720px,92vw);max-height:86vh;display:flex;flex-direction:column;box-shadow:0 10px 48px rgba(0,0,0,.6)">
        ...
        <div style="font-size:13px;color:#9fb0cc;margin-bottom:12px">选 1 只龟 → 勾技能(5选3, 调试无视等级锁) → 出战 vs 右侧 6 个假人</div>
        <div id="poc-solo-grid" style="flex:1;overflow-y:auto;display:grid;grid-template-columns:repeat(auto-fill,minmax(96px,1fr));gap:10px;padding:2px"></div>
        <div id="poc-solo-skills" style="display:none;margin-top:14px;padding-top:12px;border-top:1px solid #2a3858"></div>
        <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:10px">
          ...
          <button id="poc-solo-go" disabled style="background:#2a6f3a;color:#dfeede;border:1px solid #3e9a55;border-radius:8px;padding:9px 28px;font-size:14px;font-weight:700;cursor:pointer;opacity:.45">⚔ 开打</button>
        </div>
      </div>`;
    ov.querySelector<HTMLElement>('#poc-solo-grid')!.innerHTML = cells;
    ...
    const goBtn = ov.querySelector<HTMLButtonElement>('#poc-solo-go')!;
    const skillBox = ov.querySelector<HTMLElement>('#poc-solo-skills')!;

    const refreshGo = () => {
      const ok = !!selectedId && selectedSkills.length >= 1;
      goBtn.disabled = !ok;
      goBtn.style.opacity = ok ? '1' : '.45';
      goBtn.style.cursor = ok ? 'pointer' : 'default';
    };
    const renderSkills = () => {
      const pet = ALL_PETS.find(p => p.id === selectedId);
      if (!pet) { skillBox.style.display = 'none'; return; }
      skillBox.style.display = 'block';
      const chips = (pet.skillPool ?? []).map((s, i) => {
        const on = selectedSkills.includes(i);
        return `<button class="poc-solo-skill" data-i="${i}"
          style="padding:6px 12px;border-radius:8px;font-size:13px;cursor:pointer;transition:.12s;
          border:1px solid ${on ? '#6fa8ff' : '#3a4d76'};background:${on ? 'rgba(111,168,255,.22)' : '#1c2a47'};
          color:${on ? '#cfe3ff' : '#9fb0cc'};font-weight:${on ? 700 : 400}">${on ? '✓ ' : ''}${i + 1}. ${s.name}</button>`;
      }).join('');
      skillBox.innerHTML = `
        <div style="font-size:13px;color:#cfe3ff;margin-bottom:8px">${pet.name} — 技能 (选 ${selectedSkills.length}/3):</div>
        <div style="display:flex;flex-wrap:wrap;gap:8px">${chips}</div>`;
      skillBox.querySelectorAll<HTMLButtonElement>('.poc-solo-skill').forEach(b => {
        b.onclick = () => {
          const i = parseInt(b.dataset.i!, 10);
          if (selectedSkills.includes(i)) selectedSkills = selectedSkills.filter(x => x !== i);
          else if (selectedSkills.length < 3) selectedSkills.push(i);
          else return;   // 已满 3
          renderSkills(); refreshGo();
        };
      });
    };

    ov.querySelectorAll<HTMLButtonElement>('.poc-solo-cell').forEach(btn => {
      btn.onmouseenter = () => { if (btn.dataset.id !== selectedId) { btn.style.background = '#2a3d63'; btn.style.borderColor = '#6fa8ff'; } };
      btn.onmouseleave = () => { if (btn.dataset.id !== selectedId) { btn.style.background = '#1c2a47'; btn.style.borderColor = '#3a4d76'; } };
      btn.onclick = () => {
        selectedId = btn.dataset.id!;
        const pet = ALL_PETS.find(p => p.id === selectedId);
        // 默认勾选 defaultSkills (一般 [0,1,2]), 截 3
        selectedSkills = (pet?.defaultSkills ?? [0, 1, 2]).slice(0, 3);
        // 高亮选中 cell
        ov.querySelectorAll<HTMLButtonElement>('.poc-solo-cell').forEach(c => {
          const sel = c.dataset.id === selectedId;
          c.style.background = sel ? 'rgba(111,168,255,.28)' : '#1c2a47';
          c.style.borderColor = sel ? '#6fa8ff' : '#3a4d76';
        });
        renderSkills(); refreshGo();
      };
    });
    ...
    goBtn.onclick = () => {
      if (!selectedId || selectedSkills.length < 1) return;
      const loadout = [...selectedSkills].sort((a, b) => a - b);
  ...
  /** 「敬请期待」DOM toast (2026-05-30: 联机模式按钮未实装, 给玩家明确反馈)。
   *  复用 TeamSelectScene.showToast 风格 — 底中 fade-in / 1.8s fade-out。 */
  private showComingSoonToast(msg = '联机功能开发中, 敬请期待') {
    ...
    if (!toast) {
      ...
      toast.style.cssText = `
        ...
        color: #ffe9a8; border: 2px solid #ffd166;
    ...
    // 触发 reflow → 让 opacity transition 真生效
    void toast.offsetWidth;
    toast.style.opacity = '1';
    const t = toast as HTMLElement & { _hideTimer?: number };
    if (t._hideTimer) window.clearTimeout(t._hideTimer);
