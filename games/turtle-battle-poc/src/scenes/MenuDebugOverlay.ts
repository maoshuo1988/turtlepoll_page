...
/** 一键拥有全部龟种 — petState.pets 设为全 28 只 owned:true (主站接入时会被限制为已抽到的子集; 此按钮全解锁)。
 *  保留各龟已有的其它字段, 仅补 owned 标记。 */
function ownAllPets() {
  try {
    const ps = JSON.parse(localStorage.getItem('petState') || '{}');
    const prev: Array<{ id: string; owned?: boolean }> = Array.isArray(ps.pets) ? ps.pets : [];
    const byId = new Map(prev.map(p => [p.id, p]));
    ps.pets = ALL_PETS.map(p => ({ ...(byId.get(p.id) ?? { id: p.id }), id: p.id, owned: true }));
    localStorage.setItem('petState', JSON.stringify(ps));
  } catch { /* ignore */ }
}
/** 统计当前拥有龟种数 (pets 为空/无该字段时视作全拥有) */
function countOwned(): number {
  try {
    const ps = JSON.parse(localStorage.getItem('petState') || 'null');
    if (ps?.pets && Array.isArray(ps.pets)) return ps.pets.filter((p: { owned?: boolean }) => p.owned).length;
  } catch { /* ignore */ }
  return ALL_PETS.length;   // 无 owned 过滤 = 全部可用
}
...
export class MenuDebugOverlay {
  ...
  private renderHtml(): string {
    return `
      ...
      <div class="dbg-section">
        <h3>龟种</h3>
        <button data-action="own-all" class="success">🐢 一键拥有全部 28 龟种</button>
      </div>
  ...
  private handleAction(action: string) {
    switch (action) {
      ...
      case 'own-all':
        ...
        this.toast(`已拥有全部 ${ALL_PETS.length} 龟种`);
        break;
  ...
  private refreshInfo() {
    ...
    const lvSummary = Object.entries(lvCount)
      ...
      .map(([lv, n]) => `Lv${lv}×${n}`)
      .join('  ');
    el.textContent = `龟币: ${coins}\n拥有龟种: ${countOwned()}/${ALL_PETS.length}\n等级分布: ${lvSummary}`;
  }
