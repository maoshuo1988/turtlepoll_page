...
export class SkillTweenMgr {
  ...
  /** 停掉某个 view sprite 上的所有 active skill tween (onStop 会还原 home scale/rotation)。
   *  P???: 修"动画末尾体型突变" — 动作动画(attack/hurt)结束切回 idle 纹理时, 若起手缩放 yoyo
   *  仍在飞, 它持有的是"动作帧 scale"(~0.85); idle 帧(64px)×0.85 → 半尺寸闪一帧。
   *  纹理切换后调本方法清掉残留缩放 tween, 让 refit 设好的 home scale 立即生效 (onStop set 回 home)。 */
  stopFor(view: TweenableView) {
    const s = view.sprite;
    for (const tw of [...this.activeTweens]) {
      const targets = (tw as Phaser.Tweens.Tween & { targets?: unknown[] }).targets;
      if (Array.isArray(targets) && targets.includes(s)) {
