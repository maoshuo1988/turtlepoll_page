...
export class TurtleHud {
  ...
  private wallG: Phaser.GameObjects.Graphics | null = null;   // 石头龟 坚壁 护甲叠层进度 (黄条)
  ...
  private readonly hasStoneWall: boolean;
  ...
  constructor(scene: Phaser.Scene, b: TurtleHudBindings) {
    ...
    this.hasStoneWall = passT?.type === 'stoneWall';
    ...
    if (this.hasStoneWall) { this.wallG = scene.add.graphics(); this.container.add(this.wallG); }
  ...
  update(): void {
    ...
    const fAny = f as Fighter & {
      bubbleShieldVal?: number; _auraShieldVal?: number; _lavaShieldVal?: number; _hidingShieldVal?: number; _anemoneShield?: number;
    };
    // 熔岩盾(凤凰)/缩头盾 走 aura 段渲染 (特殊色, 区别于普通护盾段); 同一龟不会同时持两者
    const auraVal = (fAny._auraShieldVal ?? fAny._lavaShieldVal ?? fAny._hidingShieldVal ?? 0);
  ...
  private drawSpecialtyBars(): void {
    ...
    // 石头龟 坚壁: 护甲叠层进度黄条 (已叠加/上限=开局护甲×maxDefInitPct%). 底略亮, 空也可见。
    if (this.wallG) {
      ...
      const g = this.wallG;
      g.clear();
      g.fillStyle(0xffc850, 0.22); g.fillRect(x, row, w, h);
      const fw = w * Math.max(0, Math.min(1, gained / Math.max(1, maxCap)));
      if (fw > 0) { g.fillStyle(0xffd45c, 0.85); g.fillRect(x, row, fw, h); }
      row += h + 1;
    }
  }
  ...
  /** 装备图标/chip — 竖排, 龟外侧 (与 status 相反侧). 含宝箱龟从宝箱开出的专属装备(金边框)。 */
  private refreshEquipBadges(): void {
    this.clearChildren(this.equipObjs);
    const equips = (this.fighter.equipment ?? []) as Array<{ id?: string; icon?: string; name?: string }>;
    // 宝箱龟专属装备: 跟普通装备同一竖条, 金边框区分 (用户 2026-05-30 "角色身上要有装备图标和框")
    const fAny = this.fighter as Fighter & Record<string, unknown>;
    const chestExtra = (this.fighter.passive?.type === 'chestTreasure'
      ? ((fAny._chestEquips as Array<{ id?: string; icon?: string; name?: string }> | undefined) ?? [])
      : []
    ).map(eq => ({ ...eq, _isChestEquip: true }));
    const combined = [...equips, ...chestExtra];
    const onLeft = this.side !== 'left';   // equips 在 status 对面
    const edgeX = onLeft ? (this.barLeftLocalX() - 6) : (this.barLeftLocalX() + this.BAR_W + 6);
    this.layoutEquips(combined, edgeX, onLeft);
  }
   ...
   *  含宝箱龟专属装备(_isChestEquip): 用 `chest-equip-${id}` 纹理 + 金边框区分。 */
  private layoutEquips(
    equips: Array<{ id?: string; icon?: string; name?: string; _isChestEquip?: boolean }>, edgeX: number, onLeft: boolean,
  ): void {
    const box = this.CHIP_BOX, gap = 5;
    ...
    const startY = -this.LIFT - this.BAR_H / 2 - ((total - 1) * (box + gap)) / 2;
    const fAny = this.fighter as Fighter & Record<string, unknown>;
    const idOcc: Record<string, number> = {};   // 同名装备出现序号 (雷电法杖每件独立层数用)
    valid.forEach((eq, i) => {
      const cy = startY + i * (box + gap);
      const cx = onLeft ? edgeX - box / 2 : edgeX + box / 2;
      const occ = eq.id ? (idOcc[eq.id] ?? 0) : 0;
      if (eq.id) idOcc[eq.id] = occ + 1;
      const isChest = !!eq._isChestEquip;
      const texKey = eq.id ? (isChest ? `chest-equip-${eq.id}` : `equip-${eq.id}`) : '';
      const statLine = eq.id && !isChest ? getEquipStatLine(eq.id, this.fighter) : '';
      // 宝箱专属装备金边框 + 微金光晕, 跟普通装备(白线)区分
      const bg = this.scene.add.rectangle(cx, cy, box, box, 0x080c14, 0.78)
        .setStrokeStyle(isChest ? 1.5 : 1, isChest ? 0xffc850 : 0xffffff, isChest ? 0.9 : 0.18);
      ...
      // 装备特有 sub-indicator
      this.addEquipSubIndicator(eq.id, fAny, cx, cy, box, occ);
    });
  }
  ...
  private addEquipSubIndicator(
    ...
    cx: number, cy: number, box: number, occ = 0,
  ): void {
    if (id === 'e_incubator') {
    ...
    } else if (id === 'e_lightning_staff') {
      // 闪电值显示在装备框右下角 (每件独立, occ = 该件在同名装备中的序号)
      const charges = (fAny._lightningStaffCharges as number[]) ?? [];
      const charge = Math.floor(charges[occ] ?? 0);
      const t = this.scene.add.text(cx + box / 2 - 1, cy + box / 2 - 1, String(charge), {
        fontFamily: PX_FONT, fontSize: '8px', color: '#7cc3ff', stroke: '#001b3a', strokeThickness: 2,
      }).setOrigin(1, 1).setResolution(2);
      this.container.add(t);
      this.equipObjs.push(t);
    }
