...
export async function launchWaveSweep(
  scene: Phaser.Scene,
  rowKey: string,   // '0' / '1' / '2' — 行索引 (POS_BY_SLOT yPct 41/55/69), 命中同高度敌我双方 4 张 (左前+左后+右前+右后)
  allUnits: Array<{ fighter: import('../types').Fighter; sprite: Phaser.GameObjects.Sprite | Phaser.GameObjects.Image; homeY?: number; }>,
  onHit: (fighter: import('../types').Fighter) => void,
): Promise<void> {
  const sceneW = scene.scale.gameSize.width;
  const sceneH = scene.scale.gameSize.height;
  // 修(2026-05-30 v2 用户报"海浪与目标行轨迹偏离"):
  //   Agent C 上次把 endsWith('-'+col) → startsWith(rowKey+'-') 反了。装备描述明写"横排(同一高度
  //   的前后位置)" — front-0 / back-0 / 右 front-0 / 右 back-0 都在 yPct=41 这一横排;
  //   而 startsWith('front-') 拿的是 front-0/1/2 (y=41/55/69 三种高度的竖列) → mean(y) 落中间,
  //   wave 与 3 个高度的目标都对不齐。改回 endsWith('-'+rowKey) 真正抓"同高度横排"。
  //   排除中立事件生物 (JS equip-effects.js:411 同款) — 它们不在选龟队列, 不应被海浪扫到。
  const targets = allUnits
    .filter(v => v.fighter.alive
      && !(v.fighter as Fighter & { _isEventCreature?: boolean; _isNeutral?: boolean })._isEventCreature
      && !(v.fighter as Fighter & { _isEventCreature?: boolean; _isNeutral?: boolean })._isNeutral
      && v.fighter._slotKey?.endsWith('-' + rowKey))
    // homeY 优先 (避免击退/hop 中 sprite.y 漂移 → 行 y 落算错)
    .map(v => ({ fighter: v.fighter, x: v.sprite.x, y: v.homeY ?? v.sprite.y }))
    .sort((a, b) => a.x - b.x);
  // wave y = 平均 y (同行的 4 个 y 一致, 均值 = 行 y); 没目标用屏中
...
export function spawnCrystalDetonate(scene: Phaser.Scene, x: number, y: number): void {
  // 用户: 引爆动画更明显 — 中心白闪 + 双层扩散紫环 + 更多碎晶粒子 + 更强震屏
  const boom = scene.add.particles(x, y, '__DEFAULT', {
    lifespan: 520, speed: { min: 120, max: 360 }, scale: { start: 1.5, end: 0 },
    tint: [0xb478ff, 0xd8b0ff, 0x7028c8, 0xffffff], quantity: 46, blendMode: 'ADD', emitting: false,
  }).setDepth(47);
  boom.explode(46);
  // 中心白闪
  const flash = scene.add.circle(x, y, 30, 0xffffff, 0.9).setDepth(48);
  scene.tweens.add({ targets: flash, scale: 2.2, alpha: 0, duration: 200, ease: 'cubic.out', onComplete: () => flash.destroy() });
  // 双层扩散紫环
  const ring = scene.add.circle(x, y, 14, 0xb478ff, 0).setStrokeStyle(4, 0xd8b0ff, 0.9).setDepth(46);
  scene.tweens.add({ targets: ring, scale: 8, alpha: 0, duration: 460, ease: 'cubic.out', onComplete: () => ring.destroy() });
  const ring2 = scene.add.circle(x, y, 10, 0x7028c8, 0).setStrokeStyle(3, 0xb478ff, 0.7).setDepth(46);
  scene.tweens.add({ targets: ring2, scale: 5, alpha: 0, duration: 320, ease: 'cubic.out', delay: 80, onComplete: () => ring2.destroy() });
  scene.time.delayedCall(560, () => boom.destroy());
  scene.cameras.main.shake(300, 0.011);
}

// ── 贪吃蛇式彩虹光 (彩虹龟反射弹射) ─────────────────────────
// 一道彩虹光头沿弹射路径飞行, 身后留下持续的彩虹折线 (蛇身), 穿过所有目标; 结束后整条淡出。
// 用法: const s = makeRainbowSnake(scene); await s.hop(x1,y1,x2,y2) 每跳一段(到达 resolve); s.finish() 收尾淡出。
export function makeRainbowSnake(
  scene: Phaser.Scene,
): { hop: (fx: number, fy: number, tx: number, ty: number) => Promise<void>; finish: () => void } {
  const RAINBOW = [0xff3b6b, 0xff9e2c, 0xffe14d, 0x49e06b, 0x4cc9f0, 0xb06bff];
  const L = 220;   // 蛇身固定长度(px) — 头带着这段彩虹滑行, 尾随头收, 不累加全路径
  const body = scene.add.graphics().setDepth(298);
  const head = scene.add.circle(0, 0, 7, 0xffffff, 1).setDepth(301).setVisible(false);
  const glow = scene.add.circle(0, 0, 13, 0xffffff, 0.5).setDepth(300).setVisible(false);
  const path: Array<{ x: number; y: number }> = [];   // 头走过的轨迹采样
  const redraw = () => {
    body.clear();
    if (path.length < 2) return;
    // 从头(末点)往回累计到固定长度 L, 按距头距离上彩虹渐变 + 尾部渐淡
    let acc = 0;
    for (let i = path.length - 1; i > 0 && acc < L; i--) {
      const a = path[i], b = path[i - 1];
      const segLen = Math.hypot(a.x - b.x, a.y - b.y) || 0.001;
      let bx = b.x, by = b.y;
      if (acc + segLen > L) { const t = (L - acc) / segLen; bx = a.x + (b.x - a.x) * t; by = a.y + (b.y - a.y) * t; }
      const c = RAINBOW[Math.min(RAINBOW.length - 1, Math.floor(acc / (L / RAINBOW.length)))];
      const alpha = 1 - (acc / L) * 0.65;
      body.lineStyle(7, c, alpha * 0.3); body.lineBetween(a.x, a.y, bx, by);   // 外发光
      body.lineStyle(3, c, alpha); body.lineBetween(a.x, a.y, bx, by);          // 芯
      acc += segLen;
    }
  };
  return {
    hop(fx, fy, tx, ty) {
      return new Promise((resolve) => {
        if (path.length === 0) path.push({ x: fx, y: fy });
        head.setVisible(true).setPosition(fx, fy);
        glow.setVisible(true).setPosition(fx, fy);
        const dist = Math.hypot(tx - fx, ty - fy);
        const dur = Math.max(140, Math.min(320, dist * 0.6));
        scene.tweens.add({
          targets: [head, glow], x: tx, y: ty, duration: dur, ease: 'sine.inOut',
          onUpdate: () => { path.push({ x: head.x, y: head.y }); if (path.length > 240) path.shift(); redraw(); },
          onComplete: () => { path.push({ x: tx, y: ty }); redraw(); resolve(); },
        });
      });
    },
    finish() {
      scene.tweens.add({
        targets: [body, head, glow], alpha: 0, duration: 380, ease: 'cubic.out',
        onComplete: () => { body.destroy(); head.destroy(); glow.destroy(); },
      });
    },
  };
}
