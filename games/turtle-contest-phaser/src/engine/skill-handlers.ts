...
import { triggerOnHitEffects, fireCrystalDetonateVfx } from './passive-triggers';
import { fireHunterArrow } from '../vfx/skills';
...
/** v0.9.5.A77: diamondStructure flat reduce (combat.js:182-187) — 物理/魔法 减 (def×20% + mr×10%) */
function diamondFlatReduce(target: Fighter, dmgType: 'physical' | 'magic' | 'true'): number {
  if (dmgType === 'true') return 0;
  if (target.passive?.type !== 'diamondStructure') return 0;
  const enhanced = (target as Fighter & { _diamondEnhanced?: boolean })._diamondEnhanced;
  const defPct = enhanced ? 20 : ((target.passive.flatReductionPct as number) ?? 20);
  const mrPct = enhanced ? 10 : 0;
  return Math.round(target.def * defPct / 100) + Math.round((target.mr ?? 0) * mrPct / 100);
}
...
function dealPhysical(caster: Fighter, target: Fighter, base: number, isCrit: boolean): number {
  ...
  // v0.9.5.A77: diamondStructure flat reduce
  dmg = Math.max(1, dmg - diamondFlatReduce(target, 'physical'));
  ...
  const { hpLoss, shieldAbs } = applyRawDamage(target, dmg);
  let total = hpLoss + shieldAbs;
  // 真伤部分 (转换的 true 走 raw, 不受减伤)
  if (convertedTrue > 0) {
    const r2 = applyRawDamage(target, convertedTrue, 'true');
    total += (r2.hpLoss ?? 0) + (r2.shieldAbs ?? 0);
  }
  battleStats.recordDamage(caster, target, total, 'phy');
...
function dealMagic(caster: Fighter, target: Fighter, base: number): number {
  ...
  dmg = Math.max(1, dmg - diamondFlatReduce(target, 'magic'));
  const wasAlive = target.alive;
  const { hpLoss, shieldAbs } = applyRawDamage(target, dmg, 'magic');
...
export const SKILL_HANDLERS: Record<string, SkillHandler> = {
  ...
  basicChiWave: async (api, caster, target, skill) => {
    ...
    // P154 1:1 JS basic.js:12-19 — 单个 chiWaveActive buff (含 4 项加成 + 显示 1 个 💥 图标),
    //   不是 4 个分开 buff (PoC 旧版显示 4 个图标; JS 只显 1 个). recalcStats 读它应用加成.
    caster.buffs.push({
      type: 'chiWaveActive', value: 0, duration: 2,
    ...
    api.floatNum(caster, '⚡蓄气中...', '#58a6ff');
  ...
  basicSlam: async (api, caster, target, skill) => {
    ...
    scene.tweens.chain({
      ...
      onComplete: () => { tv.sprite.x = tHomeX; tv.sprite.y = tHomeY; tv.sprite.angle = 0; },
    });
  ...
  angelSmite: async (api, caster, _target, skill) => {
    ...
    if (stealAmt > 0) {
      ...
      api.floatNum(tgt, `-${stealAmt}护甲魔抗`, '#ef4444', 'debuff-num');
  ...
  ghostPhantom: async (api, caster, target, skill) => {
    ...
    const isCrit = rollCrit(effectiveCrit(caster));
    const critMult = isCrit ? calcCritMult(caster) : 1;
    const base = caster.atk * ((skill.atkScale as number) ?? 1.5) * critMult;
    ...
    applyPostHitLayers(caster, target, dmg, isCrit, 'magic');
  ...
  ghostStorm: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      ...
      if (hasCurse) {
        ...
        const s = dealMagic(caster, target, base);
        totalDmg += s;
    ...
    if (!hasCurse) {
      target.buffs.push({ type: 'curse', value: Math.round(target.maxHp * 0.05), duration: dotTurns });
  ...
  phoenixShield: async (api, caster, _target, skill) => {
    ...
    caster.buffs.push({ type: 'counter', value: Math.round(caster.atk * counterScale), duration: dur + 1 });
    api.floatNum(caster, `+${shieldAmt}`, '#c0c0c0', 'shield-num');
    api.log?.(`${caster.emoji}${caster.name} <b>熔岩盾</b>: +${shieldAmt}护盾 ${dur}回合 + 受击反击 ${Math.round(caster.atk * counterScale)} 魔法`);
    return { touched: [caster] };
  },
  ...
  phoenixPurify: async (api, caster, target, _skill) => {
    ...
    const debuffTypes = ['atkDown', 'defDown', 'mrDown', 'burn', 'curse', 'healReduce', 'poison', 'bleed', 'armorBreak'];
  ...
  shellCopy: async (api, caster, _target, _skill) => {
    const BLACKLIST = new Set([
      ...
      'diceFate', 'fortuneDice', 'fortuneAllIn', 'bambooHeal', 'bambooLeaf',
      'ghostPhase', 'diamondFortify', 'twoHeadSwitch', 'mechAttack',
  ...
  shellErode: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < waveCount; i++) {
      if (target.alive) totalMain += dealMagic(caster, target, caster.atk * mainScale);
      if (otherInCol?.alive) {
        totalBehind += dealMagic(caster, otherInCol, caster.atk * behindScale);
        if (!touched.includes(otherInCol)) touched.push(otherInCol);
      }
      // P17 JS shell.js:281 — 每波间 80ms (浮字位移)
      if (i < waveCount - 1) await sleep(80);
    }
    api.floatNum(target, `${totalMain}`, '#4cc9f0', 'magic-dmg');
    if (otherInCol) api.floatNum(otherInCol, `${totalBehind}`, '#4cc9f0', 'magic-dmg');
  ...
  diamondSmash: async (api, caster, target, skill) => {
    ...
    if (bleedTurns && target.alive) {
      ...
      api.floatNum(target, `流血×${stacks}`, '#cc3333', 'debuff-num');
    }
  ...
  diamondCollide: async (api, caster, target, skill) => {
    ...
    // Collision counter — JS uses attacker._diamondCollideCount[tIdx] (per target).
    // Phaser: 用同款字段, key 走 target.id (没 allFighters idx, 用 id+side 唯一).
    const stunAfter = (skill.stunAfter as number) ?? 0;
    if (stunAfter > 0 && target.alive) {
      const att = caster as Fighter & { _diamondCollideCount?: Record<string, number> };
      if (!att._diamondCollideCount) att._diamondCollideCount = {};
      const key = `${target.side}-${target.id}`;
      att._diamondCollideCount[key] = (att._diamondCollideCount[key] ?? 0) + 1;
      if (att._diamondCollideCount[key] >= stunAfter) {
        att._diamondCollideCount[key] = 0;
  ...
  // ── stone ──
  // JS stone.js:3-12 doStoneShield 1:1 — selfCast 单体自盾
  //   shield = round(maxHp × shieldHpPct% × shieldMult)  (默认 shieldHpPct=20)
  //   shieldMult = 铁壁之日 ×1.3, 否则 ×1
  stoneShield: async (api, caster, _target, skill) => {
    const shieldHpPct = (skill.shieldHpPct as number) ?? 20;
    const amt = Math.round(Math.round(caster.maxHp * shieldHpPct / 100) * ruleModifiers.shieldMult());
    applyShield(caster, amt);
    api.floatNum(caster, `+${amt}`, '#c0c0c0', 'shield-num', 0, 0);
    await sleep(800);  // P80 JS stone.js:11
    return { touched: [caster] };
  },
  ...
  bambooHeal: async (api, caster, _target, skill) => {
    ...
    if (allies.length > 0) {
      ...
      // 给友军 hidingShield buff (healPct=0, JS bamboo.js:39 — 到期无 heal, 纯护盾)
      for (const a of allies) {
        const shieldAmt = Math.round(caster.maxHp * shieldPct / 100);
        a.shield = (a.shield ?? 0) + shieldAmt;
        // P81 1:1 JS bamboo.js:39 — type='hidingShield' shieldVal/healPct/turns 不是 'shield'
        a.buffs.push({ type: 'hidingShield', value: shieldAmt, duration: shieldTurns + 1, healPct: 0 });
  ...
  bambooSmack: async (api, caster, target, skill) => {
    ...
    if (chilled && target.alive) {
      ...
      api.floatNum(target, '❄冰寒', '#87ceeb', 'debuff-num', 200, 0);
      api.log?.(`${target.emoji}${target.name} 被冰寒 ${chilled} 回合`);
    }
    // P18 knockToFront: JS bamboo.js:187-219 1:1 — 找空前排槽 + 改 _position/_slotKey
    if (knockToFront && target.alive && target._position === 'back') {
      const enemyTeam = api.allFighters.filter(f => f.side === target.side);
      const frontCount = enemyTeam.filter(f => f.alive && f._position === 'front').length;
      if (frontCount < 3) {
        // 找空前排槽 — JS:194 used slot 用其他 alive 占的
        const used = new Set(enemyTeam.filter(f => f.alive && f !== target).map(f => (f as Fighter & { _slotKey?: string })._slotKey));
        const frontSlots = ['front-0', 'front-1', 'front-2'];
        const emptyFront = frontSlots.find(s => !used.has(s));
        target._position = 'front';
        if (emptyFront) (target as Fighter & { _slotKey?: string })._slotKey = emptyFront;
        api.floatNum(target, '击至前排!', '#7dffb3');
        // 视觉: 真正把 sprite 0.4s 平移到新前排槽 (JS bamboo.js:197-214)
        api.repositionFighter?.(target);
      }
    }
    return { touched: [target] };
  },
  ...
  diceFate: async (api, caster, _target, skill) => {
    ...
    api.floatNum(caster, `🎲 +${critGain}%💥 ${duration}t`, '#ffd93d');
  ...
  hunterMark: async (api, caster, target, skill) => {
    ...
    if (target.alive && skill.markTurns) {
      ...
      api.floatNum(target, '🎯猎杀印记', '#ff8c42', 'debuff-num');
  ...
  lavaSplash: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      if (!target.alive) break;
      const dmg = dealMagic(caster, target, caster.atk * atkScale);
      api.floatNum(target, `${dmg}`, '#4dabf7', 'magic-dmg');
      if (i < hits - 1) await sleep(500);  // E3/8: JS combat.js:266 同款 500ms 间隔
    }
    // applySkillDebuffs({burn:true}) 默认层数 round(atk×0.67) (JS lava.js:77)
    if (skill.burn && target.alive) applyBurn(target, defaultBurnStacks(caster));
    api.log?.(`${caster.emoji}${caster.name} 熔岩飞溅 → ${target.emoji}${target.name} ${hits}段${skill.burn ? ' + 灼烧' : ''}`);
    return { touched: [target] };
  },
  ...
  lightningSurgeBuff: async (api, caster, target, skill) => {
    ...
    battleStats.recordDamage(caster, target, shown, 'tru');
    if (wasA && !target.alive) battleStats.recordKill(caster, target);
    api.floatNum(target, `⚡${shown}`, '#ffd93d', 'pierce-dmg');
    triggerOnHitEffects(caster, target, dmg, { floatNum: api.floatNum });
  ...
  crystalBarrier: async (api, caster, _target, skill) => {
    ...
    api.floatNum(caster, `+${shieldAmt}🔮`, '#4cc9f0', 'shield-num');
  ...
  crystalBurst: async (api, caster, _target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      for (const e of enemies) {
        ...
        api.floatNum(e, `${mDmg}`, '#4dabf7', 'magic-dmg');
  ...
  cyberBeam: async (api, caster, target, skill) => {
    ...
    const hopMs = 460;
    // 抛物: y(t) = yShift × t + 4·apexLift·t·(1-t)
    const hopTweens: Array<{ y: number; duration: number; ease: string }> = [];
    const steps = 8;
    for (let i = 1; i <= steps; i++) {
      const t = i / steps;
      const y = homeYC + yShift * t + apexLift * 4 * t * (1 - t);
      hopTweens.push({ y, duration: hopMs / steps, ease: 'linear' });
    }
    scene.tweens.chain({ targets: cv.sprite, tweens: hopTweens });
    cv.sprite.setDepth(50);
    await sleep(480);
    ...
    scene.tweens.add({
      targets: cv.sprite, scale: cv.sprite.scaleX * 1.08,
    ...
    const hitTasks = enemyList.map(async (enemy) => {
      ...
      for (let seg = 0; seg < 2; seg++) {
        ...
        // P33: chi-hit-flash per segment (JS cyber.js:270-272 .chi-hit-flash 140ms tint)
        if ('setTint' in ev.sprite) {
          (ev.sprite as Phaser.GameObjects.Sprite).setTint(0xffffff);
          scene.time.delayedCall(140, () => {
            try { (ev.sprite as Phaser.GameObjects.Sprite).clearTint(); } catch { /* ignore */ }
          });
        ...
        if (trueDmgPerSeg > 0 && enemy.alive) {
          ...
          // P33: yOffset +24 (JS cyber.js:265 spawnFloatingNum yOff 24) — true 在 phys 下方
    ...
    // 抛回 home (反向抛物)
    const hopBackTweens: Array<{ x: number; y: number; duration: number; ease: string }> = [];
    for (let i = 1; i <= steps; i++) {
      const t = i / steps;
      const y = (homeYC + yShift) - yShift * t + apexLift * 4 * t * (1 - t);
      hopBackTweens.push({ x: homeXC, y, duration: hopMs / steps, ease: 'linear' });
    }
    scene.tweens.chain({ targets: cv.sprite, tweens: hopBackTweens });
  ...
  lineFinish: async (api, caster, target, skill) => {
    ...
    // 飘字: 引爆提示
    if (stacks > 0) api.floatNum(target, `墨迹×${stacks} 引爆!`, '#ffd93d');
    // 物理伤
    ...
    triggerOnHitEffects(caster, target, normalDmg + burstDmg, { floatNum: api.floatNum });
  ...
  bubbleBurst: async (api, caster, target, skill) => {
    ...
    for (const e of targets) {
      ...
      if (magicDmg > 0) {
        const wasA = e.alive;
        const r = applyRawDamage(e, magicDmg, 'magic');
        const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
  ...
  iceFrost: async (api, caster, _target, skill) => {
    ...
    if (mrDown) {
      for (const e of enemies) {
        ...
        e.mr = Math.max(0, e.mr - Math.round((e.baseMr ?? e.mr) * mrDown.pct / 100));
        api.floatNum(e, `❄抗-${mrDown.pct}%`, '#4cc9f0');
      }
      await sleep(300);
    }
    ...
    for (let i = 0; i < hits; i++) {
      for (const e of enemies) {
        ...
        const critMult = isCrit ? calcCritMult(caster) : 1;
        const effMr = calcEffMr(caster, e);
        const dmg = Math.max(1, Math.round(perHit * critMult * calcDmgMult(effMr) * ruleModifiers.magicMult()));
        const wasA = e.alive;
  ...
  iceFreeze: async (api, caster, target, skill) => {
    ...
    if (target.alive) {
      target.buffs.push({ type: 'stun', value: 1, duration: 2 });
      (target as Fighter & { _stunUsed?: boolean })._stunUsed = false;
      api.floatNum(target, '💫眩晕', '#fbbf24', 'debuff-num');
  ...
  iceSpike: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      ...
      triggerOnHitEffects(caster, target, dmg, { floatNum: api.floatNum });
      await sleep(500);   // JS ice.js:72 hit-shake 500ms
      await sleep(150);   // JS:74 间隔 150ms
    }
  ...
  fortuneDice: async (api, caster, _target, skill) => {
    ...
    api.floatNum(caster, `🎲${roll} +${roll}💰`, '#ffd93d');
    // Heal % maxHp
    ...
    for (let i = 0; i < hits; i++) {
      ...
      if (hasChain) {
        ...
        if (others.length) {
          ...
          api.floatNum(sec, `${sShown}🔗`, color, dmgType === 'true' ? 'true-dmg' : 'direct-dmg');
  ...
  hidingDefend: async (api, caster, _target, skill) => {
    ...
    caster.buffs.push({
      type: 'hidingShield',
      value: shieldAmt,
      duration: shieldDuration + 1,
    });
    // 存到 fighter 字段供到期 hook 用 (TODO: BattleScene 端处理)
    (caster as Fighter & { _hidingShieldHealPct?: number })._hidingShieldHealPct = shieldHealPct;
    api.floatNum(caster, `+${shieldAmt} ${shieldDuration}t`, '#c0c0c0', 'shield-num');
    api.log?.(`${caster.emoji}${caster.name} <b>防御</b>: +${shieldAmt}护盾 ${shieldDuration}回合 (到期回收 ${shieldHealPct}% HP)`);
    return { touched: [caster] };
  },
  ...
  headlessStorm: async (api, caster, _target, skill) => {
    ...
    const cf = caster as Fighter & { _lifestealPct?: number };
    const origLifesteal = cf._lifestealPct ?? 0;
    cf._lifestealPct = origLifesteal + tempLifesteal;
    let totalDmg = 0;
    for (const e of enemies) {
      if (!e.alive) continue;
      for (let h = 0; h < hits; h++) {
        ...
        if (wasA && !e.alive) battleStats.recordKill(caster, e);
        api.floatNum(e, `${shown}`, '#ff4444', 'direct-dmg');
        // 生命偷取 (cf._lifestealPct 已临时 +22%)
        if (caster.alive && shown > 0) {
          const heal = Math.round(shown * cf._lifestealPct / 100);
          const before = caster.hp;
    ...
    cf._lifestealPct = origLifesteal;
    void totalDmg;
    return { touched: enemies };
  },
  ...
  twoHeadSwitch: async (api, caster, target, skill) => {
    ...
    if (switchTo === 'melee') {
      ...
      // switch-attack: 对 HP 最低敌人 1.2× ATK
  ...
  starBeam: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      ...
      const effMr = calcEffMr(caster, target);
      let dmg = Math.max(1, Math.round(baseDmg * critMult * calcDmgMult(effMr) * ruleModifiers.magicMult()));
      // wormhole bonus
      const wh = target.buffs.find(b => b.type === 'wormhole'
        && (b as typeof target.buffs[number] & { sourceIdx?: number }).sourceIdx === api.allFighters.indexOf(caster));
      if (wh) {
        const normalBonusPct = (wh as typeof wh & { normalBonusPct?: number }).normalBonusPct ?? 0;
        dmg = Math.round(dmg * (1 + normalBonusPct / 100));
      }
      const wasA = target.alive;
      const r = applyRawDamage(target, dmg, 'magic');
      const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
  ...
  starMeteor: async (api, caster, _target, skill) => {
    ...
    if (p?.type === 'starEnergy') {
      ...
      if ((cf._starEnergy ?? 0) >= maxE) {
        ...
        for (const e of enemies) {
          if (!e.alive) continue;
          const wh = e.buffs.find(b => b.type === 'wormhole') as typeof e.buffs[number] & { pierceBonusPct?: number } | undefined;
          const finalDmg = wh && wh.pierceBonusPct ? Math.round(burstDmg * (1 + wh.pierceBonusPct / 100)) : burstDmg;
          const wasA = e.alive;
        ...
        api.floatNum(caster, '⭐ 星能爆发!', '#ffd93d');
  ...
  starBlackhole: async (api, caster, target, skill) => {
    ...
    if (isExecute) {
      ...
      if (wasA && !target.alive) battleStats.recordKill(caster, target);
      api.floatNum(target, '🎯黑洞斩杀!', '#ff2255');
    } else {
      ...
      if (!isLastEnemy && target.alive) {
        ...
        scene.time.delayedCall(1100, () => {
          tv.sprite.clearTint();
          (target as Fighter & { _isInBlackhole?: boolean })._isInBlackhole = false;
        });
      }
    }
  ...
  starGravityWarp: async (api, caster, _target, skill) => {
    ...
    if (p?.type === 'starEnergy') {
      ...
      if ((cf._starEnergy ?? 0) >= maxE) {
        ...
        cf._starEnergy = 0;
        api.floatNum(caster, '⭐扭曲! 换位', '#ffd93d');
  ...
  // v0.9.5.B7: starWormhole 真 port — JS star.js:100-125
  // 虫洞: target 加 wormhole buff (pierceBonusPct + normalBonusPct), caster 永久 +magicPen
  starWormhole: async (api, caster, target, skill) => {
    if (!target) return { touched: [] };
    const pierceBonusPct = (skill.pierceBonusPct as number) ?? 25;
    const normalBonusPct = (skill.normalBonusPct as number) ?? 15;
    const duration = (skill.duration as number) ?? 3;
    const magicPenAtkPct = (skill.magicPenAtkPct as number) ?? 0;
    // 移除旧 wormhole (相同 sourceId)
    const sourceIdx = api.allFighters.indexOf(caster);
    target.buffs = target.buffs.filter(b => !(b.type === 'wormhole'
      && (b as typeof target.buffs[number] & { sourceIdx?: number }).sourceIdx === sourceIdx));
    target.buffs.push({
      type: 'wormhole', value: pierceBonusPct, duration: duration + 1,
      pierceBonusPct, normalBonusPct, sourceIdx,
    } as typeof target.buffs[number] & { pierceBonusPct: number; normalBonusPct: number; sourceIdx: number });
    api.floatNum(target, `🌀虫洞 +${pierceBonusPct}%真 ${duration}t`, '#c77dff');
    // 永久 +magicPen
    if (magicPenAtkPct > 0) {
      const penGain = Math.round(caster.atk * magicPenAtkPct / 100);
      caster.magicPen = (caster.magicPen ?? 0) + penGain;
      api.floatNum(caster, `+${penGain}🔮穿`, '#4cc9f0');
    }
    // P89 1:1 JS star.js:123 — fireStarPassive 收尾 (之前 PoC 缺)
  ...
  volcanoStomp: async (api, caster, _target, skill) => {
    ...
    for (const e of enemies) {
      ...
      if (Math.random() * 100 < stunChance && e.alive) {
        ...
        api.floatNum(e, '眩晕', '#ffd86b', 'debuff-num', 200, 0);
      }
      triggerOnHitEffects(caster, e, dmg, { floatNum: api.floatNum });
      touched.push(e);
    }
  ...
  gamblerBet: async (api, caster, target, skill) => {
    ...
    api.floatNum(caster, `${hpCost}HP`, '#ff4444', 'direct-dmg');   // JS:75 '${hpCost}HP' direct-dmg
    api.log?.(`${caster.emoji}${caster.name} 赌注! 消耗 ${hpCost}HP`);   // JS:77
    await sleep(500);   // JS:78
    ...
    let totalDmg = 0;
    for (let i = 0; i < hits; i++) {
      if (!target.alive) continue;   // JS:89 keep animating remaining
      const isCrit = rollCrit(effectiveCrit(caster));
      const critMult = isCrit ? calcCritMult(caster) : 1;
      const eDef = calcEffArmor(caster, target);
      ...
      if (wasA && !target.alive) battleStats.recordKill(caster, target);
      api.floatNum(target, `${shown}`, '#ff4444', isCrit ? 'crit-dmg' : 'direct-dmg');
      // JS:100 triggerOnHitEffects (含 tryGamblerMultiHit chain — passive-triggers.ts:451)
      triggerOnHitEffects(caster, target, dmg, { floatNum: api.floatNum });
      applyPostHitLayers(caster, target, shown, isCrit, 'physical');
      await sleep(500);   // JS:101 hit-shake 500ms
      await sleep(100);   // JS:103 100ms 间隔
    }
  ...
  // P28: gamblerCards 严格 1:1 JS combat.js:828-852.
  //   - hits 段, 每段随机 minScale~maxScale × ATK 物理
  //   - **JS 不 crit cards** (无 critRoll/critMult) — 之前 PoC 误加 crit, 改回
  //   - hit-shake 700ms + 200ms 间隔 + tryGamblerMultiHit chain (passive proc)
  //   - 飘字始终 'direct-dmg' (JS:849 spawnFloatingNum 写死 'direct-dmg')
  gamblerCards: async (api, caster, target, skill) => {
    ...
    for (let i = 0; i < hits; i++) {
      ...
      const eDef = calcEffArmor(caster, target);
      // JS: dmg = max(1, round(atk × scale × calcDmgMult(eDef))) — NO crit multiplier
      const dmg = Math.max(1, Math.round(caster.atk * scale * calcDmgMult(eDef)));
      const wasA = target.alive;
      const r = applyRawDamage(target, dmg, 'physical');
      const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
      total += shown;
      battleStats.recordDamage(caster, target, shown, 'phy');
      if (wasA && !target.alive) battleStats.recordKill(caster, target);
      // JS:849 写死 'direct-dmg', 不 crit
      api.floatNum(target, `${shown}`, '#ff4444', 'direct-dmg');
      triggerOnHitEffects(caster, target, dmg, { floatNum: api.floatNum });
      applyPostHitLayers(caster, target, shown, false, 'physical');  // JS no-crit
  ...
  hunterPoison: async (api, caster, target, skill) => {
    ...
    if (dot && target.alive) {
      ...
      api.floatNum(target, `☠×${stacks}`, '#9b59b6', 'debuff-num');
    }
  ...
  diceAllIn: async (api, caster, _target, skill) => {
    ...
    let totalDmg = 0;
    api.floatNum(caster, '孤注一掷!', '#ffd93d');
    const baseRaw = Math.round(caster.atk * atkScale);
    for (const e of enemies) {
      if (!e.alive) continue;
      const isCrit = rollCrit(effectiveCrit(caster));
  ...
  diceFlashStrike: async (api, caster, _target, skill) => {
    ...
    api.floatNum(caster, `🎲${roll}点 → ${totalSegs}段!`, '#ffd93d');
    let totalDmg = 0;
  ...
  rainbowReflect: async (api, caster, _target, skill) => {
    ...
    if (caster.alive) {
      ...
      api.floatNum(caster, `+${val()}🌈`, '#3deb9e');
    ...
    while (guard++ < 40) {
      ...
      if (next === 'enemy') {
        const e = getEnemies(api, caster).find(x => x.alive && !hitEnemies.has(x));
        if (!e) break;
        const isCrit = rollCrit(effectiveCrit(caster));
        const critMult = isCrit ? calcCritMult(caster) : 1;
        ...
        const a = getAllies(api, caster).find(x => x.alive && !healedAllies.has(x));
        if (!a) break;
        applyHeal(a, val(), caster);
        api.floatNum(a, `+${val()}🌈`, '#3deb9e');
  ...
  lineInkBomb: async (api, caster, _target, skill) => {
    ...
    const hits = (skill.hits as number) ?? 3;
    const atkScale = (skill.atkScale as number) ?? 0.3;
    const inkStacks = (skill.inkStacks as number) ?? 2;
    const baseDmg = Math.round(caster.atk * atkScale);
    for (const e of enemies) {
      if (!e.alive) continue;
      const eDef = calcEffArmor(caster, e);
      ...
      addInkStack(e, inkStacks, caster);   // 含 _inkLink 伙伴同步
      const ee = e as Fighter & { _inkStacks?: number };
      api.floatNum(e, `+${inkStacks}🖌 (${ee._inkStacks})`, '#c77dff');
    }
    return { touched: enemies };
  },
  ...
  lineLink: async (api, caster, target, skill) => {
    ...
    if (second) {
      ...
      (target as Fighter & { _inkLink?: { partner: Fighter; turns: number; transferPct: number; dmgType: 'magic' | 'true' } })._inkLink = {
        partner: second, turns: duration, transferPct, dmgType: linkType,
      };
      (second as Fighter & { _inkLink?: { partner: Fighter; turns: number; transferPct: number; dmgType: 'magic' | 'true' } })._inkLink = {
        partner: target, turns: duration, transferPct, dmgType: linkType,
      };
  ...
  headlessSoulStrike: async (api, caster, target, skill) => {
    ...
    const baseDmg = Math.round(caster.atk * atkScale);
    const hpDmg = Math.round(target.hp * hpPct / 100);
    ...
    battleStats.recordDamage(caster, target, shown, 'mag');
    if (wasA && !target.alive) battleStats.recordKill(caster, target);
    api.floatNum(target, `${shown}`, '#4dabf7', 'magic-dmg');
    triggerOnHitEffects(caster, target, totalDmg, { floatNum: api.floatNum });
    return { touched: [target] };
  },
  ...
  soulReap: async (api, caster, _target, skill) => {
    ...
    const lifestealPct = (skill.lifestealPct as number) ?? 0;
    const lostHp = caster.maxHp - caster.hp;
    const baseDmg = Math.round(caster.atk * atkScale) + Math.round(lostHp * lostHpPct / 100);
    let totalDmg = 0;
    for (const e of enemies) {
      if (!e.alive) continue;
      const isCrit = rollCrit(effectiveCrit(caster));
  ...
  bubbleShield: async (api, caster, target, skill) => {
    ...
    const tt = tgt as Fighter & { bubbleShieldVal?: number; bubbleShieldTurns?: number; bubbleShieldOwner?: Fighter };
    ...
    api.floatNum(tgt, `+${amount}🫧 ${duration}t`, '#4cc9f0');
    return { touched: [tgt] };
  },
  ...
  hidingCommand: async (api, caster, _target, _skill) => {
    ...
    // 发事件给 BattleScene 让随从额外行动一次
    api.scene.events.emit('hiding-command', { owner: caster, summon });
    return { touched: [caster, summon] };
  },

  // ── angel ──
  // v0.9.5.B7: angelRevive 用 skill.reviveHpPct 字段
  angelRevive: async (api, caster, _target, skill) => {
    const reviveHpPct = (skill.reviveHpPct as number) ?? 30;
    const dead = api.allFighters.filter(f => !f.alive && f.side === caster.side);
    if (dead.length === 0) {
      api.floatNum(caster, '无死亡友军', '#888');
      return { touched: [caster] };
    }
    const tgt = dead[0];
    tgt.alive = true;
    tgt.hp = Math.round(tgt.maxHp * reviveHpPct / 100);
    tgt.shield = 0;
    tgt.buffs = []; // 清状态
    api.floatNum(tgt, `复活 +${tgt.hp}`, '#06d6a0', 'heal-num');
    return { touched: [tgt] };
  },
  ...
  angelEquality: async (api, caster, target, skill) => {
    ...
    if (target.alive && isHighRarity) {
      ...
      if (wasA && !target.alive) battleStats.recordKill(caster, target);
      // P82 1:1 JS angel.js:83 — delay 100, yOff 22
      api.floatNum(target, `${shown3}`, '#ffffff', isCrit ? 'crit-true' : 'true-dmg', 100, 22);
  ...
  fortuneGainCoins: async (api, caster, _target, skill) => {
    ...
    api.floatNum(caster, `+${gain}💰 (${cf._goldCoins})`, '#ffd93d');
  ...
  fortuneBuyEquip: async (api, caster, _target, skill) => {
    const cost = (skill.coinCost as number) ?? 20;
    const cf = caster as Fighter & { _goldCoins?: number };
    const coins = cf._goldCoins ?? 0;
    if (coins < cost) {
      api.floatNum(caster, `金币不足 (${coins}/${cost})`, '#888');
      return { touched: [caster] };
    }
    cf._goldCoins = coins - cost;
    // BattleScene 端 push 进 bench (用 scene event)
    api.scene.events.emit('fortune-buy-equip', { side: caster.side });
    api.floatNum(caster, `🎰抽装备! -${cost}💰`, '#ffd93d');
    sfxCoin();  // E3/22: JS fortune.js coin spend
    return { touched: [caster] };
  },

  // fortuneAllIn: 消耗所有金币 → 每枚 perCoinAtkNormal × ATK 物理 + perCoinAtkPierce × ATK 真伤
  fortuneAllIn: async (api, caster, target, skill) => {
    if (!target) return { touched: [] };
    const cf = caster as Fighter & { _goldCoins?: number };
    const coins = cf._goldCoins ?? 0;
    if (coins <= 0) {
      api.floatNum(caster, '没金币!', '#888');
      return { touched: [target] };
    }
    cf._goldCoins = 0;
    ...
    for (let i = 0; i < coins; i++) {
      ...
      if (piercePer > 0 && target.alive) {
        ...
        const pShown = (rp.hpLoss ?? 0) + (rp.shieldAbs ?? 0);
        totalP += pShown;
      }
      battleStats.recordDamage(caster, target, nShown + piercePer, 'phy');
      if (wasA && !target.alive) battleStats.recordKill(caster, target);
      api.floatNum(target, `${nShown}`, '#ff4444', 'direct-dmg');
      if (piercePer > 0) api.floatNum(target, `${piercePer}`, '#ffffff', 'true-dmg');
      triggerOnHitEffects(caster, target, nDmg + piercePer, { floatNum: api.floatNum });
      await sleep(perDelay);
    }
    api.floatNum(target, `${coins}枚梭哈! 共-${totalN + totalP}`, '#ffd93d');
  ...
  heal: async (api, caster, target, skill) => {
    const tgt = (target && target.side === caster.side) ? target : caster;
    // 即时治疗: 仅当无 HoT 时即时回 (纯 HoT 技能如海盗朗姆酒只持续回, 不即时回)
    if (!skill.hot) {
      const amt = Math.round(caster.atk * ((skill.atkScale as number) ?? 1.0));
    ...
    if (defBuff && (defBuff.pct ?? 0) > 0) {
      const gain = Math.round((tgt.baseDef ?? tgt.def) * (defBuff.pct ?? 0) / 100);
    ...
    if (mrBuff && (mrBuff.pct ?? 0) > 0) {
      const gain = Math.round((tgt.baseMr ?? tgt.mr ?? 0) * (mrBuff.pct ?? 0) / 100);
  ...
  twoHeadFear: async (api, caster, target, skill) => {
    ...
    if (target.alive) {
      ...
      await sleep(200);
      api.floatNum(target, '恐惧!', '#ff9f43', 'debuff-num');
    }
