...
import { calcDamage, applyRawDamage, rollCrit, calcCritMult, calcEffMr, calcEffArmor, calcDmgMult } from '../engine/damage';
...
import { applyTeamSynergies, SYNERGY_TAGS, type ActiveSynergy } from '../data/synergies';
import { fireOnHit, fireOnTurnBegin, fireOnDeath } from '../engine/equipment-runtime';
import { triggerOnHitEffects, resetPerTurnFlags, resetPerCastFlags, setLightningVfxHook, setCrystalDetonateVfxHook, setChestTreasureHook, setStaffChargeHook } from '../engine/passive-triggers';
...
import { getSkillHandler, setSkillApi, spawnLightningStrike, applyHeal, type BattleApi } from '../engine/skill-handlers';
...
interface FighterView {
  ...
  lightningBadge?: Phaser.GameObjects.Text;   // 雷电法杖实时闪电值 (⚡N), 仅携带者
...
export class BattleScene extends Phaser.Scene {
  ...
  private benchPickerUI?: Phaser.GameObjects.Container;               // E2: 目标选择 modal
  ...
  private async showInitialEquipPickModal(): Promise<string | null> {
    return new Promise<string | null>((resolve) => {
      ...
      const GOLD = '#c8aa6e', LGOLD = '#f0e6d2', TEAL = '#0ac8b9';
      ...
      head.style.cssText = 'text-align:center;margin-bottom:28px';
      head.innerHTML = `
        <div style="color:${TEAL};font-size:12px;letter-spacing:7px;text-transform:uppercase;margin-bottom:8px">⬡ 装备锻造 ⬡</div>
        <div style="color:${LGOLD};font-size:26px;font-weight:600;letter-spacing:3px;text-shadow:0 0 16px rgba(10,200,185,.45)">选择你的初始装备</div>
        <div style="margin:13px auto 0;width:240px;height:1px;background:linear-gradient(90deg,transparent,${GOLD},transparent)"></div>
      `;
      overlay.appendChild(head);

      const grid = document.createElement('div');
      grid.style.cssText = 'display:flex;gap:28px;justify-content:center;align-items:stretch';
      pool.forEach((id, i) => {
        const eq = EQUIP_BY_ID[id]; if (!eq) return;
        const frame = document.createElement('div');   // 金色角切外框
        frame.style.cssText = `
          width:212px;cursor:pointer;clip-path:${CLIP};padding:2px;box-sizing:border-box;
          background:linear-gradient(155deg,${GOLD},#785a28 50%,#243744);
          transition:transform .16s ease, filter .16s ease;
          transform:translateY(18px);opacity:0;
        `;
        const inner = document.createElement('div');   // 深蓝内卡
        inner.style.cssText = `
          clip-path:${CLIP};height:100%;box-sizing:border-box;
          background:linear-gradient(180deg,#0a2530,#06131f 60%,#040f1a);
          padding:18px 14px 16px;text-align:center;
        `;
        const iconSrc = eq.icon?.endsWith('.png') ? eq.icon : '';
        inner.innerHTML = `
          <div style="width:104px;height:104px;margin:0 auto 10px;display:flex;align-items:center;justify-content:center;
               background:radial-gradient(circle,rgba(10,200,185,.18),transparent 68%)">
            ${iconSrc ? `<img src="${iconSrc}" style="width:94px;height:94px;object-fit:contain;image-rendering:pixelated;filter:drop-shadow(0 2px 7px rgba(0,0,0,.6))"/>` : `<div style="font-size:54px">${eq.icon ?? '📦'}</div>`}
          </div>
          <div style="color:${LGOLD};font-size:17px;font-weight:700;letter-spacing:1px;margin-bottom:5px">${eq.name}</div>
          <div style="margin:0 auto 11px;width:62%;height:1px;background:linear-gradient(90deg,transparent,${TEAL},transparent)"></div>
          <div style="color:#a8c4cc;font-size:11.5px;line-height:1.55;text-align:left;white-space:pre-line">${eq.desc ?? ''}</div>
        `;
        frame.appendChild(inner);
        frame.onmouseover = () => { frame.style.transform = 'translateY(0) scale(1.05)'; frame.style.filter = `drop-shadow(0 0 18px rgba(10,200,185,.6))`; };
        frame.onmouseout = () => { frame.style.transform = 'translateY(0) scale(1)'; frame.style.filter = 'none'; };
        frame.onclick = () => finish(id);
        grid.appendChild(frame);
        setTimeout(() => { frame.style.transform = 'translateY(0)'; frame.style.opacity = '1'; }, 90 + i * 95);
      });
  ...
  create() {
    ...
    this.benchRail = new BenchRail(this, {
      onSlotClick: (side, idx, eq) => {
        // 训龟大师的口哨: 特殊物品 → 点击「吹响」(不弹目标 picker)
        if ((eq as { actionable?: string }).actionable === 'blow') {
          this.blowMasterWhistle(idx, side);
          return;
        }
        // 点击回退: 弹目标选择 modal (拖拽是主路径)
        this.showBenchPicker(eq, idx, side);
      },
    ...
    const placeTeam = (team: string[], slots: string[], side: 'left' | 'right', autoSort = false) => {
      ...
      const sideLevel = side === 'left' ? this.leftLevel : this.rightLevel;
      const built = team.map((id) => {
        const equippedIdxs = (side === 'left' && this.loadouts[id]) ? this.loadouts[id] : undefined;
        const f = createFighter(id, side, (equippedIdxs || sideLevel != null) ? { equippedIdxs, level: sideLevel } : undefined);
      ...
      if (pet?.skillPool) {
        for (const i of equippedIdxs) {
          ...
          if (s?.type === 'gamblerEnhancedMulti') {
            ...
            this.battleLog?.log(`🎰 ${f.name} 强化多重打击 (-30% maxHp, 概率 60%)`);
          }
          ...
          if (s?.type === 'diceGamblerConvert') {
            ...
            f.baseDef = 0; f.def = 0;
            f.baseMr = 0; f.mr = 0;
            (f as Fighter & { _diceGamblerConverted?: boolean })._diceGamblerConverted = true;
  ...
  private makeView(f: Fighter, x: number, y: number): FighterView {
    ...
    // P19 用户: 影子放大 (80→104 / 24→32) + 上移到脚底 (之前偏低, 龟图底部留白使视觉脚底高于接地点)
    const sw = 104 * baseScale, sh = 32 * baseScale;
    const shLift = Math.round(12 * baseScale);
    const shadow = this.textures.exists('fx-shadow')
      ? (this.add.image(x, y - shLift, 'fx-shadow').setDisplaySize(sw, sh).setDepth(1) as unknown as Phaser.GameObjects.Ellipse)
      : this.add.ellipse(x, y - shLift, sw, sh, 0x000000, 0.40).setDepth(1);
    // JS drop-shadow(0 0 3px) 柔边
    const shadowFx = (shadow as unknown as { preFX?: { addBlur?: (...args: number[]) => void } }).preFX;
    if (shadowFx && typeof shadowFx.addBlur === 'function') {
      shadowFx.addBlur(0, 3, 3, 1, 0x000000);
    }
    ...
    const view: FighterView = {
      fighter: f, sprite, shadow, hpBar, hpBarBg, hpBarHi, shieldBar, hpDelayBar, hpText, nameText, statusGroup,
  ...
  private isNonActor(f: Fighter): boolean {
    const c = f as Fighter & {
      ...
      _isConchWorm?: boolean; _isEventCreature?: boolean;
    };
    if (c._isCrystalBall || c._isCandyBomb || c._isPirateShip || c._isConchWorm || c._isEventCreature) return true;
  ...
  private startTurnTimer(canAct: FighterView[]): void {
    ...
    this.topRow.setTurnTimer(this._turnTimerLeft);
    this._turnTimerEvent = this.time.addEvent({
      delay: 1000, loop: true, callback: () => {
        this._turnTimerLeft--;
        this.topRow.setTurnTimer(Math.max(0, this._turnTimerLeft));
  ...
  private nextActor() {
    ...
    if (isBossSide) {
      ...
      // boss 眩晕/恐惧 → 整回合跳过
      if (bossView.fighter.buffs.some(b => (b.type === 'stun' || b.type === 'fear') && !bossView.fighter._stunUsed)) {
        bossView.fighter._stunUsed = true;
        bossView.fighter.buffs = bossView.fighter.buffs.filter(b => b.type !== 'stun' && b.type !== 'fear');
    ...
    // v0.9.5.A75: stun/fear 跳过 (JS turn.js:1237) — 检 canAct 中所有 stun/fear, 跳过 + 消耗
    const stunned = canAct.filter(v =>
      v.fighter.buffs.some(b => (b.type === 'stun' || b.type === 'fear') && !v.fighter._stunUsed)
    );
    if (stunned.length > 0) {
      for (const sv of stunned) {
        this.actedThisSide.add(sv);
        sv.fighter._stunUsed = true;
        sv.fighter.buffs = sv.fighter.buffs.filter(b => b.type !== 'stun' && b.type !== 'fear');
  ...
  private startActorTurn(actor: FighterView) {
    ...
    {
      const uf = actor.fighter as Fighter & { _undeadLockTurns?: number };
      if ((uf._undeadLockTurns ?? 0) > 0) {
        ...
        if ((uf._undeadLockTurns as number) <= 0) {
          ...
          spawnFloatingText(this, actor.sprite.x, actor.sprite.y - 20, '锁血结束', 'debuff-label', {});
        }
      }
    }
  ...
  private enterTargetingMode(actor: FighterView, skillIdx: number, kind: 'enemy' | 'ally') {
    ...
    let candidates = this.views.filter(v =>
      v.fighter.alive && (kind === 'enemy' ? v.fighter.side !== actor.fighter.side : v.fighter.side === actor.fighter.side)
    );
  ...
  private runEnemyAIImmediate(actor: FighterView) {
    ...
    let pool = enemies.filter(e => !e.fighter.buffs.some(b => b.type === 'stealth'));
    if (pool.length === 0) pool = enemies; // 全员 stealth 则破例
    ...
    return {
      ...
      floatNum: (target, text, color, explicitCls, delayMs, yOffset) => {
        ...
        if (explicitCls) {
          ...
          else cls = isCrit ? 'crit-dmg' : 'direct-dmg';
        }
  ...
  playHitKnockback(tv: FighterView, attackerSide: 'left' | 'right'): void {
    ...
    const syncFollow = () => {
      if (tv.shadow) tv.shadow.x = tv.sprite.x;
        ...
        this.tweens.add({
          ...
          onComplete: () => {
            ...
            this.tweens.add({
              ...
              onComplete: () => { tv.sprite.x = homeX; tv.sprite.y = homeY; if (tv.shadow) tv.shadow.x = homeX; },
            });
          },
        });
      },
  ...
  private repositionViewToSlot(f: import('../types').Fighter): void {
    ...
    this.tweens.add({
      ...
      onUpdate: () => {
        if (v.shadow) v.shadow.x = v.sprite.x;
        v.sceneTurtleDom?.setCanvasPos(v.sprite.x, v.sprite.y);
      },
      onComplete: () => {
        if (v.shadow) { v.shadow.x = c.x; v.shadow.y = shadowY; }
        v.sceneTurtleDom?.setCanvasPos(c.x, newSpriteY);
      },
    });
  }
   ...
   */
  /** 雷电法杖: 每段主动伤害命中 → 充能(单体+25/AOE+12.5) + 刷新实时层数; 满100立刻发射连锁闪电(减100留溢出)。 */
  private onLightningStaffHit(attacker: Fighter, isAoe: boolean): void {
    const af = attacker as Fighter & { _lightningStaffCharge?: number };
    af._lightningStaffCharge = (af._lightningStaffCharge ?? 0) + (isAoe ? 12.5 : 25);
    while ((af._lightningStaffCharge ?? 0) >= 100) {
      af._lightningStaffCharge = (af._lightningStaffCharge ?? 0) - 100;
      this.fireLightningChain(attacker);
    }
    this.updateLightningBadge(attacker);
  }

  /** 雷电法杖实时层数 badge (⚡N) — 挂携带者头顶, 每次充能即时刷新, 玩家随时看到层数变化。 */
  private updateLightningBadge(fighter: Fighter): void {
    const v = this.views.find(x => x.fighter === fighter);
    if (!v) return;
    if (!v.fighter.alive) { v.lightningBadge?.setVisible(false); return; }
    const charge = Math.floor((fighter as Fighter & { _lightningStaffCharge?: number })._lightningStaffCharge ?? 0);
    const bx = v.sprite.x;
    const by = v.sprite.y - (v.sprite.displayHeight ?? 80) / 2 - 30;
    if (!v.lightningBadge) {
      v.lightningBadge = this.add.text(bx, by, '', {
        fontSize: '13px', color: '#7cc3ff',
        fontFamily: 'm6x11, pixel-zh, "Microsoft YaHei", system-ui',
        stroke: '#001b3a', strokeThickness: 3,
      }).setOrigin(0.5).setDepth(46);
    }
    v.lightningBadge.setText(`⚡${charge}`).setPosition(bx, by).setVisible(true);
  }
  ...
  private fireLightningChain(attacker: Fighter): void {
    ...
    for (let i = 0; i < 4 && pool.length; i++) {
      ...
      // VFX 链式跳跃 (略 stagger); 伤害已即时结算, 数字与血条同步刷新
      this.time.delayedCall(delay, () => {
        this.drawLightningBolt(sx, sy, ex, ey);
        this.spawnFloatingPassive(tv, `⚡${shown}`, '#7cc3ff');
        this.tweens.add({ targets: tv.hpBar, width: 118 * Math.max(0, t.hp / t.maxHp), duration: 150 });
        tv.hpText.setText(`${Math.max(0, t.hp)}/${t.maxHp}`);
  ...
  private async fireOnCastEquips(actor: FighterView, target: FighterView | null, touched: Fighter[], skill: SkillDef): Promise<void> {
    ...
    if ((cf._stunBatonStacks ?? 0) > 0) {
      const stunTarget = isSingle && target?.fighter.alive ? target.fighter : pickRandomEnemy();
      if (stunTarget?.alive) {
    ...
    if (cf._turtleSword === true) {
      const swordTarget = isSingle && target?.fighter.alive ? target.fighter : pickFrontEnemy();
  ...
  private async fireBambooChargeIfReady(actor: FighterView, target: FighterView | null) {
    ...
    this.spawnFloatingPassive(tv, '🎋充能!', '#10b981');
      ...
      if (!skipPosHop) {
        ...
        const counter = this.tweens.addCounter({
          ...
          onUpdate: (tween) => {
            ...
            if (actor.shadow) actor.shadow.x = homeX + dx;
          },
          onComplete: () => {
            // 兜底
            actor.sprite.x = homeX;
            actor.sprite.y = homeY;
            if (actor.shadow) actor.shadow.x = homeX;
  ...
  private processDeathPassives(deadView: FighterView) {
      ...
      this.reviveFighter(deadView, {
        ...
        onRevive: () => {
          ...
          for (const eView of enemies) {
            ...
            if (existingHR) {
              existingHR.duration = 3;
            } else {
              e.buffs.push({ type: 'healReduce', value: 50, duration: 3 });
            }
    ...
    if ((dead as Fighter & { _equipConch?: boolean; _conchUsed?: boolean })._equipConch
      ...
      dead.name = '小虫'; dead.emoji = '🐛';
      // 替换技能 (JS equip-effects.js:198-200)
      dead.skills = [{
    ...
    if (dead.passive?.type === 'cyberDrone' && !(dead as Fighter & { _mechFormed?: boolean })._mechFormed) {
      ...
      if (dc > 0) {
        ...
        dead.baseDef = 0; dead.def = 0; dead.baseMr = 0; dead.mr = 0;
    ...
    const hookPct = pType === 'deathHook' ? (dead.passive!.pct as number)
                  ...
                  : 0;
    if (hookPct > 0 && this.currentAttacker && this.currentAttacker.alive) {
      const attacker = this.currentAttacker;
      const attackerView = this.views.find(v => v.fighter === attacker);
    ...
    for (const fg of fortunes) {
      ...
      // 飘字 (JS:430): '+9🪙' passive-num delayMs=500
  ...
  private processChestTreasureGain(view: FighterView, gain: number) {
    ...
    const thresholds = (p.thresholds as number[]) ?? [80, 130, 240, 360, 590];
  ...
  private applyChestEquipStat(view: FighterView, equip: { stat: string; pct?: number; bonusHp?: number }) {
    ...
    switch (equip.stat) {
      ...
      case 'hot':
        ...
        break;
      case 'star':
        // P87 1:1 修命名: skill-handlers.ts 读 _chestEquipStar (之前 BattleScene 写 _equipChestStar 不一致)
        //   → 4 个装备分支 (star/thunder/chain/rock) 全 dead, 用户报"重点看宝箱龟这么多装备都要过一遍"
  ...
  private async castWithAnnounce(actor: FighterView, target: FighterView | null, skillIdx: number) {
    ...
    await this.runSkillHandler(actor, target, skillIdx);
    this.endTurn();
  }
  ...
  private triggerEnterPassives() {
    for (const v of this.views) {
      ...
      // v0.9.5.A57: frostAura — 登场冰寒, 全敌方 atkDown 6 回合
      if (p.type === 'frostAura') {
        const pct = (p.atkDownPct as number) ?? 20;
        const turns = (p.atkDownTurns as number) ?? 6;
        const enemies = this.views.filter(x => x.fighter.alive && x.fighter.side !== f.side);
        for (const e of enemies) {
          e.fighter.buffs.push({ type: 'atkDown', value: pct, duration: turns + 1 });
        }
        if (enemies.length) this.battleLog.log(`❄️ ${f.name} <b>冰寒</b>: 敌方 ATK -${pct}% ${turns} 回合`);
      }
  ...
  private processTurnBeginPassives(view: FighterView) {
    ...
    const { expired } = tickBuffsDuration(f);
    // P18: hidingShield buff 到期 → 回 shieldHealPct% × 剩余 shieldVal HP (JS hiding.js:1-13)
    for (const b of expired) {
      if (b.type === 'hidingShield') {
        const shieldVal = (b.value as number) ?? 0;
        // P81 1:1 JS turn.js:946-960 — healPct 优先从 buff 读 (bambooHeal:0, hiding 龟:caster._hidingShieldHealPct)
        const ff = f as Fighter & { _hidingShieldHealPct?: number };
        const buffHealPct = (b as { healPct?: number }).healPct;
        const pct = (typeof buffHealPct === 'number') ? buffHealPct : (ff._hidingShieldHealPct ?? 50);
        // JS: heal = round(min(剩余 caster.shield, shieldVal) × pct / 100)
        const remaining = Math.min(f.shield ?? 0, shieldVal);
        const heal = Math.round(remaining * pct / 100);
        if (heal > 0 && f.alive) {
          const before = f.hp;
          ...
          if (actual > 0) {
            ...
            this.spawnFloatingPassive(view, `+${actual}`, '#06d6a0');   // P131 删自创❤+"隐身回血"标签, heal-num cls
            this.battleLog.log(`🛡 ${f.name} <b>防御盾过期</b> → 回 ${actual} HP`);
          }
        }
      }
    }
    ...
    if (hotPct > 0 && f.alive) {
      const heal = Math.round(f.maxHp * hotPct / 100);
      const before = f.hp;
      f.hp = Math.min(f.maxHp, f.hp + heal);
      const actual = f.hp - before;
      if (actual > 0) {
    ...
    // fortuneGold: 每回合 +2 龟币 (用户 v0.9.9). 玩家方加 this.coins, 野生敌方走 aiGainCoins.
    if (p.type === 'fortuneGold') {
      if (f.side === 'left') {
        this.coins += 2;
        this.refreshCoinDisplay();
        this.battleLog.log(`💰 ${f.name} <b>财神</b> +2 龟币`);
      } else {
    ...
    // P34 stoneWall: 每回合永久 +def (JS turn.js:147-166)
    //   capTurns 6 回合达到 maxDefInitPct (100%) × _initDef
    //   每回合 _stoneDefFraction += maxCap/capTurns, 实际给 round 后 diff
    if (p.type === 'stoneWall') {
      ...
      if (ff._stoneDefGained < maxCap) {
        ...
        if (gain > 0) {
          f.baseDef += gain;
          f.def = f.baseDef;   // recalc 之后 buff 再 apply
          ff._stoneDefGained = target;
          this.spawnFloatingPassive(view, `+${gain}🪨甲`, '#a0c8ff');
    ...
    // v0.9.5.A50: auraAwaken (shell 气场觉醒) — JS 对齐:
    //   第 4 回合 (awakenTurn): 全属性 +12% (atk/def/mr/hp/lifesteal/reflect) + crit +25%
    //   储能: 受伤时累积 (上限 50% maxHp); 每 4 回合 (energyReleaseTurn) 引爆: 对全体敌方造成储能×40%+1%/级 物理 + 自己获得储能×80% 气场护盾
    //   enhancedAwakenTurn (8): 强化觉醒 — 同款效果再次触发 (需 enhanced 被动技能)
    if (p.type === 'auraAwaken') {
      ...
      const doAwaken = (label: string) => {
        f.baseAtk = Math.round(f.baseAtk * (1 + atkPct / 100));
        f.atk = f.baseAtk;
        f.baseDef = Math.round(f.baseDef * (1 + defPct / 100));
        f.def = f.baseDef;
        f.baseMr  = Math.round((f.baseMr ?? f.mr) * (1 + mrPct / 100));
        f.mr = f.baseMr;
        const newMaxHp = Math.round(f.maxHp * (1 + hpPct / 100));
        const hpDelta = newMaxHp - f.maxHp;
        f.maxHp = newMaxHp; f.hp += hpDelta;
        (f as Fighter & { lifestealPct?: number }).lifestealPct =
          ((f as Fighter & { lifestealPct?: number }).lifestealPct || 0) + lifestealPct / 100;
        (f as Fighter & { reflectPct?: number }).reflectPct =
          ((f as Fighter & { reflectPct?: number }).reflectPct || 0) + reflectPct / 100;
        f.crit = (f.crit || 0) + critGain;
        this.battleLog.log(`✨ ${f.name} ${label}! +${atkPct}% 全属性 + ${critGain * 100}% 暴击`);
      };
      if (turn === awakenTurn) doAwaken('气场觉醒');
      // enhancedAwaken: 仅当 fighter 装备 "强化觉醒" passiveSkill 时第 8 回合再次触发
      if (turn === enhancedTurn && f._passiveSkills?.some?.((ps: { type: string }) => ps.type === 'shellEnhanceAwaken')) {
        doAwaken('强化觉醒');
      }
      // 储能波击移到回合末 processEnergyWave() (JS chest.js:209 在 turn.js:1365 round-end 调),
      //   不再在 shell turn-begin 重复释放 (避免双波 + 对齐 JS 时机)。
    }
    ...
    // candySteal — 指定回合 D&D Life Drain (JS turn.js:259-289)
    const stealTurn = (p.stealTurn as number) ?? 3;
    if (p.type === 'candySteal' && this.turn === stealTurn) {
      const enemies = this.views.filter(v => v.fighter.side !== f.side && v.fighter.alive).map(v => v.fighter);
      if (enemies.length) {
        const target = enemies[Math.floor(Math.random() * enemies.length)];
        const stealPct = (p.stealPct as number) ?? 10;
        const stealAmt = Math.round(target.maxHp * stealPct / 100);
        ...
        if (tv) {
          this.spawnFloatingPassive(tv, `-${stealAmt}🍬`, '#ffd93d');
    ...
    // rainbowPrism — 每回合随机色光全队 buff (JS turn.js:397-444)
    if (p.type === 'rainbowPrism') {
      this.applyRainbowPrism(view);
    }
    ...
    // gamblerFateWheel — 4 花色抽属性 (1:1 JS turn.js:318-344)
    //   ♠ +5攻+30HP / ♥ +2甲+2魔抗 / ♦ +8%暴击+2穿甲 / ♣ +4%吸血; 累计 _fateWheelCounts
    if (p.type === 'gamblerFateWheel') {
      const ff = f as Fighter & {
        _fateWheelCounts?: { spade: number; heart: number; diamond: number; club: number };
        _initHp?: number; _lifestealPct?: number;
      };
      if (!ff._fateWheelCounts) ff._fateWheelCounts = { spade: 0, heart: 0, diamond: 0, club: 0 };
      const suit = Math.floor(Math.random() * 4); // 0♠ 1♥ 2♦ 3♣
      if (suit === 0) {
        ff._fateWheelCounts.spade++;
        f.baseAtk += 5; f.atk += 5;
        f.maxHp += 30; f.hp += 30; ff._initHp = f.maxHp;
        ...
        f.baseDef += 2; f.def += 2;
        f.baseMr = (f.baseMr ?? f.baseDef) + 2; f.mr += 2;
        this.spawnFloatingPassive(view, '♥ +2甲+2魔抗', '#ef4444');
      } else if (suit === 2) {
        ff._fateWheelCounts.diamond++;
        // ♦ 永久 +暴击/+穿甲 — JS recalc 不重置 crit/armorPen, PoC recalc 会, 故同步 _base 快照
  ...
  private processComplexEquipEffects(actor: FighterView) {
    ...
    if (f._equipFpga) {
      ...
      if (state === 0) {
        ...
        // 10: this-turn +15% dmg (buff)
        f.buffs.push({ type: 'atkUp', value: Math.round(f.baseAtk * 0.15), duration: 2 });
        ...
        // 11: this-turn -25% taken (buff, physImmune-like)
        f.buffs.push({ type: 'physImmune', value: 25, duration: 2 });
    ...
    if (f._equipAmplifier) {
      ...
      f.buffs.push({ type: 'atkUp', value: Math.round(f.baseAtk * pct / 100), duration: 2 });
    ...
    // ── 迷你水晶球 A: 回合末 2 段魔法光线 ──
    if (f._equipMiniCrystal && enemies.length) {
      // 随机选一列敌人 (简化: 1 个目标)
      const t = enemies[Math.floor(Math.random() * enemies.length)];
      // K1: 水晶光线 VFX (JS drawCrystalBeam, 与水晶球召唤同款) — 先射线再落伤害
      castCrystalBeam(this, actor.sprite.x, actor.sprite.y, t.sprite.x, t.sprite.y);
      for (let i = 0; i < 2; i++) {
        this.time.delayedCall(260 + i * 180, () => {
          if (!t.fighter.alive) return;
          this.dealMagicHit(t, 30, '💎');
          this.addMiniCrystalStack(t);
        });
      }
    }
    // ── 迷你水晶球 B: 全敌方各 20 法术 ──
    if (f._equipMiniCrystalB) {
      enemies.forEach((ev, i) => {
        this.time.delayedCall(i * 100, () => {
          if (!ev.fighter.alive) return;
          this.dealMagicHit(ev, 20, '🔴');
          this.addMiniCrystalStack(ev);
        });
      });
    }
  ...
  private triggerDragonFly(actor: FighterView, allies: FighterView[], enemies: FighterView[]) {
    ...
    for (const a of colAllies) {
      ...
      a.fighter.hp = Math.min(a.fighter.maxHp, a.fighter.hp + heal);
    ...
    for (const ev of colEnemies) {
      this.dealMagicHit(ev, 50, '🐉');
      applyDotStacks(ev.fighter, 'burn', 25);
      this.refreshStatusIcons(ev);
    }
  }

  /** 迷你水晶球: 给目标 +1 层, 满 3 层立即引爆 14% maxHp 魔法 */
  private addMiniCrystalStack(view: FighterView) {
    const f = view.fighter as Fighter & { _miniCrystalStacks?: number };
    f._miniCrystalStacks = (f._miniCrystalStacks ?? 0) + 1;
    this.spawnFloatingPassive(view, `💎${f._miniCrystalStacks}`, '#c77dff');
    if (f._miniCrystalStacks >= 3) {
      f._miniCrystalStacks = 0;
      const dmg = Math.round(f.maxHp * 0.14);
      this.dealMagicHit(view, dmg, '💥');
      spawnCrystalDetonate(this, view.sprite.x, view.sprite.y);   // K2: 紫色径向爆 + 震屏
      this.battleLog.log(`💥 ${f.name} 水晶引爆! ${dmg} 法术`);
    }
  }

  /** 通用 dealMagicHit (复杂装备共用) */
  private dealMagicHit(view: FighterView, dmg: number, emoji: string) {
    const f = view.fighter;
    if (!f.alive) return;
    const wasAlive = f.alive;
    const r = applyRawDamage(f, dmg, 'magic');
    const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
    battleStats.recordDamage(null, f, shown, 'mag');
  ...
  /** 通用 dealPhysicalHit (海螺小虫共用) */
  private dealPhysicalHit(view: FighterView, dmg: number, emoji: string) {
    const f = view.fighter;
    if (!f.alive) return;
    const wasAlive = f.alive;
    const r = applyRawDamage(f, dmg, 'physical');
    const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
    battleStats.recordDamage(null, f, shown, 'phy');
    this.spawnFloatingPassive(view, `${shown}${emoji}`, '#ff4444');   // 伤害统一无符号
    this.tweens.add({ targets: view.hpBar, width: 118 * (f.hp / f.maxHp), duration: 200 });
    view.hpText.setText(`${f.hp}/${f.maxHp}`);
    if (wasAlive && !f.alive) this.killView(view);
  }
  ...
  private applyRainbowPrism(view: FighterView) {
    ...
    for (const color of picks) {
      if (color === 0) {  // 红: 全队 atk +%
        ...
        const t = enemies[Math.floor(Math.random() * enemies.length)];
        t.fighter.buffs.push({ type: 'curse', value: Math.round(t.fighter.maxHp * 0.09), duration: 3 });
        this.refreshStatusIcons(t);
      }
      this.spawnFloatingPassive(view, NAMES[color] ?? '?', '#ffffff');
    }
  ...
  private async processHunterExecute(): Promise<void> {
    for (const v of this.views) {
      ...
      for (const ev of enemies) {
        ...
        // Phase 1: icon overlay (放大消失) 700ms
        const icon = this.add.text(ev.sprite.x, ev.sprite.y - 30, '🎯', {
          fontSize: '64px', stroke: '#000', strokeThickness: 4,
        }).setOrigin(0.5).setDepth(60).setScale(0.3);
        this.tweens.add({
          targets: icon, scale: 1, alpha: 0.95, duration: 280, ease: 'back.out',
        });
        this.time.delayedCall(600, () => {
          this.tweens.add({
            targets: icon, scale: 1.6, alpha: 0, duration: 280, ease: 'cubic.out',
        ...
        this.spawnFloatingPassive(ev, '🎯 猎杀!', '#ff2222');
  ...
  private processLavaRage(view: FighterView) {
    ...
    // 切 volcanoSkills (从 PET_BY_ID 拿)
    const petDef = (PET_BY_ID as Record<string, { volcanoSkills?: typeof f.skills }>)[f.id];
    if (petDef?.volcanoSkills) {
      f.skills = petDef.volcanoSkills
        .filter(s => !s.passiveSkill)
        .slice(0, 3)
        .map(s => ({ ...s, cdLeft: 0 }));
    }
    f.name = '火山龟';
    ...
    const burst = this.add.particles(sx, sy, '__DEFAULT', {
      ...
      tint: [0xff4400, 0xff8800, 0xffdd33], quantity: 60, blendMode: 'ADD', emitting: false,
    }).setDepth(45);
    burst.explode(60);
  ...
  private spawnSummonAlly(owner: FighterView, passive: { hpPct?: number; maxRarity?: string }) {
    ...
    const summon = createFighter(pick.id, ownerF.side);
    const sHp = Math.round(ownerF.maxHp * hpPct / 100);
    summon.maxHp = sHp;
    summon.hp = sHp;
  ...
  private processCandyBombDecay(): void {
    for (const v of [...this.views]) {
      ...
      b.hp = Math.max(0, b.hp - dec);
      this.spawnFloatingPassive(v, `-${dec}🍬`, '#ff6bd6');
      this.updateHpVisual(v, { duration: 200 });
      if (b.hp <= 0) {
        b.alive = false;
  ...
  private detonateCandyBomb(bomb: Fighter): void {
    ...
    if (bv) {
      const boom = this.add.particles(bv.sprite.x, bv.sprite.y, '__DEFAULT', {
        ...
        tint: [0xff6bd6, 0xffd93d, 0xff5050], quantity: 36, blendMode: 'ADD', emitting: false,
      }).setDepth(48);
      boom.explode(36);
    ...
    const perDmg = Math.max(1, Math.round(totalDmg / enemies.length));
    this.battleLog.log(`💥 糖果炸弹引爆！全体敌方分摊 ${totalDmg} 法术 (每只 ${perDmg})`);
    for (const e of enemies) {
      if (!e.alive) continue;
      const r = applyRawDamage(e, perDmg, 'magic');
      const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
    ...
    for (const ballView of balls) {
      ...
      for (let seg = 0; seg < 2; seg++) {
        for (const e of inPath) {
          if (!e.alive) continue;
          const r = applyRawDamage(e, segDmgBase, 'magic');
          const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
          battleStats.recordDamage(ball, e, shown, 'mag');
          const ev = this.views.find(v => v.fighter === e);
          if (ev) {
            spawnFloatingText(this, ev.sprite.x, ev.sprite.y, `${shown}`, 'magic-dmg',
              { amount: shown, atkSide: ball.side === 'left' ? 'right' : 'left' });
          }
          // 加 1 层 _crystallize (与水晶龟本体共享)
          const ee = e as Fighter & { _crystallize?: number };
          ee._crystallize = (ee._crystallize ?? 0) + 1;
  ...
  private processPirateShipFire(allFighters: Fighter[]) {
    for (const f of allFighters) {
      ...
      // 简化伤害公式 (def 减免)
      const eff = target.def - (f.armorPen || 0);
      const final = Math.max(1, Math.round(dmg * (eff >= 0 ? (1 - eff / (eff + 100)) : (1 + Math.abs(eff) / (Math.abs(eff) + 100)))));
      const r = applyRawDamage(target, final, 'physical');
      const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
      const view = this.views.find(v => v.fighter === target);
      if (view) {
        this.tweens.add({ targets: view.hpBar, width: 118 * (target.hp / target.maxHp), duration: 200 });
        view.hpText.setText(`${target.hp}/${target.maxHp}`);
        const t = this.add.text(view.sprite.x, view.sprite.y - 40, `-${shown}💥`, {
          fontSize: '18px', color: '#ff8800', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui', fontStyle: 'bold',
          stroke: '#000', strokeThickness: 3,
        }).setOrigin(0.5).setDepth(50);
        this.tweens.add({
          targets: t, y: view.sprite.y - 90, alpha: 0, duration: 800, ease: 'cubic.out',
          onComplete: () => t.destroy(),
        });
  ...
  private masterChiWave(view: FighterView): void {
    ...
    for (const tv of targets) {
      ...
      if (total > 0) this.spawnFloatingPassive(tv, `-${total}`, '#ff6b6b');
      this.syncHpView(tgt);
  ...
  private async processSideEnd(endedSide: 'left' | 'right'): Promise<void> {
    ...
    for (const v of lightningOwners) {
      ...
      const shockDmg = Math.round(v.fighter.atk * shockScale);
    ...
    for (const v of droneOwners) {
      ...
      if (spawned > 0) {
        spawnFloatingText(this, v.sprite.x, v.sprite.y, `+${spawned}🛸`, 'passive-num');
    ...
    for (const cw of conchOwners) {
      ...
      this.dealPhysicalHit(lowest, Math.round(cf.atk * 1.0), '🐛');
  ...
  private async processSideEndEquipment(endedSide: 'left' | 'right'): Promise<void> {
    ...
    for (const v of this.views) {
      ...
      // E3/23 wave 海浪 (JS:832-863): 每 3 回合横扫一行 (front/back random)
      ...
      if (fAny._equipWave) {
        ...
        if (fAny._equipWaveStacks >= 3) {
          ...
          const rowKey: 'front' | 'back' = Math.random() < 0.5 ? 'front' : 'back';
          ...
          this.battleLog.log(`🌊 ${f.name} 海浪 横扫 ${rowKey === 'front' ? '前排' : '后排'}`);
  ...
  private async processRoundEndBuffs(): Promise<void> {
    ...
    for (const view of this.views) {
      const f = view.fighter as Fighter & {
        ...
        bubbleShieldTurns?: number; bubbleShieldVal?: number; bubbleShieldOwner?: Fighter | null;
        ...
        if ((f.bubbleShieldTurns ?? 0) <= 0 && (f.bubbleShieldVal ?? 0) > 0) {
          ...
          if (owner && owner.alive) {
            ...
            for (const ev of enemies) {
              ...
              const r = applyRawDamage(ev.fighter, burstDmg);
              const shown = (r.hpLoss ?? 0) + (r.shieldAbs ?? 0);
              battleStats.recordDamage(owner, ev.fighter, shown, 'phy');
              if (wasAlive && !ev.fighter.alive) {
                battleStats.recordKill(owner, ev.fighter);
                this.killView(ev);
              }
              this.spawnFloatingPassive(ev, `${shown}💧爆`, '#4cc9f0');
            }
            this.battleLog.log(`${f.emoji}${f.name} 泡泡盾自然破碎! 对敌方全体 ${burstDmg} 物理`);
            hadTick = true;
          }
          f.bubbleShieldVal = 0;
    ...
    if (f.passive?.type === 'bubbleStore' && ((f.bubbleStore as number) ?? 0) > 0) {
      ...
      if (dmgPct > 0) {
        ...
        if (dmgAmt > 0) {
          ...
          if (enemies.length > 0) {
            const t = enemies[Math.floor(Math.random() * enemies.length)];
            const finalDmg = Math.max(1, Math.round(dmgAmt));
  ...
  private tickDoTs(f: import('../types').Fighter): number {
    ...
    for (const b of f.buffs) {
      ...
      if (dmg > 0) {
        ...
        battleStats.recordDamage(null, f, shown, statCat);
        // P139: DoT 飘字 1:1 JS turn.js:679 — 只显实际伤害数字 (shown, 含护盾吸收), 无自创 emoji
        if (view && shown > 0) {
          const t = this.add.text(view.sprite.x + (Math.random() - 0.5) * 30, view.sprite.y - 50, `-${shown}`, {
  ...
  /** E2: 显示目标选择 modal — 点装备后弹出选哪只龟装 (JS bench.js drag-drop UX 的简化版) */
  private showBenchPicker(eq: import('../types').EquipmentDef, benchIdx: number, side: 'left' | 'right' = 'left') {
    if (this.benchPickerUI) { this.benchPickerUI.destroy(); this.benchPickerUI = undefined; }
    const { width, height } = this.scale.gameSize;
    const container = this.add.container(width / 2, height / 2).setDepth(2000);
    this.benchPickerUI = container;

    // 半透明遮罩 (点击关闭)
    const veil = this.add.rectangle(0, 0, width, height, 0x000000, 0.6)
      .setInteractive();
    veil.on('pointerdown', () => this.closeBenchPicker());
    container.add(veil);

    // 主面板
    const panelW = 420, panelH = 280;
    const panel = this.add.rectangle(0, 0, panelW, panelH, 0x1a2740, 0.97)
      .setStrokeStyle(3, 0xffd93d);
    container.add(panel);

    // 标题
    container.add(this.add.text(0, -panelH / 2 + 24, `装备 ${eq.icon ?? '📦'} ${eq.name}`, {
      fontSize: '18px', color: '#ffd93d', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui', fontStyle: 'bold',
    }).setOrigin(0.5));
    container.add(this.add.text(0, -panelH / 2 + 48, '选择目标', {
      fontSize: '13px', color: '#aaa', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui',
    }).setOrigin(0.5));

    // P128: 目标列表 — 1:1 JS bench.js tryEquipFromBench L66-68:
    //   isConsumable + target:'enemy' → 显示对面 (c_bomb 打敌方);
    //   否则 (装备/ally 消耗品) 显示本侧.
    //   之前一律 side === side 过滤 → c_bomb 永远没法选敌方目标, 用户报"物品用不了".
    const isConsumable = eq.category === 'consumable';
    const wantsEnemy = isConsumable && (eq as { target?: string }).target === 'enemy';
    const targetSide: 'left' | 'right' = wantsEnemy ? (side === 'left' ? 'right' : 'left') : side;
    const targets = this.views.filter(v => v.fighter.side === targetSide && v.fighter.alive);
    const cellW = 110, gap = 12;
    const totalW = cellW * targets.length + gap * (targets.length - 1);
    const startX = -totalW / 2 + cellW / 2;
    targets.forEach((view, i) => {
      const cx = startX + i * (cellW + gap);
      const cy = 0;
      const fit = this.validateEquipFit(view.fighter, eq);
      const ringColor = fit.ok ? (wantsEnemy ? 0xff6b6b : 0x4ade80) : 0x888888;
      const cell = this.add.rectangle(cx, cy, cellW, 140, 0x0a0e18, 0.95).setStrokeStyle(2, ringColor);
      container.add(cell);
      container.add(this.add.image(cx, cy - 30, `pet-${view.fighter.id}`).setDisplaySize(56, 56)
        .setAlpha(fit.ok ? 1 : 0.45));
      container.add(this.add.text(cx, cy + 8, view.fighter.name, {
        fontSize: '12px', color: fit.ok ? '#fff' : '#888', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui',
      }).setOrigin(0.5));
      container.add(this.add.text(cx, cy + 26, `${view.fighter.hp}/${view.fighter.maxHp}`, {
        fontSize: '11px', color: '#aaa', fontFamily: 'monospace',
      }).setOrigin(0.5));
      container.add(this.add.text(cx, cy + 42, `🎒${view.fighter.equipment?.length ?? 0}/10`, {
        fontSize: '11px', color: '#aaa', fontFamily: 'monospace',
      }).setOrigin(0.5));
      if (!fit.ok) {
        container.add(this.add.text(cx, cy + 58, fit.reason ?? '', {
          fontSize: '10px', color: '#ff5050', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui',
        }).setOrigin(0.5));
      }
      if (fit.ok) {
        cell.setInteractive({ useHandCursor: true });
        cell.on('pointerover', () => cell.setStrokeStyle(3, 0xffd93d));
        cell.on('pointerout', () => cell.setStrokeStyle(2, ringColor));
        cell.on('pointerdown', () => {
          // 注意: side 仍是 bench 源侧 (用于 splice bench); fighter 是 targetSide 上的目标
          this.applyBenchEquipToFighter(view.fighter, eq, benchIdx, side);
          this.closeBenchPicker();
        });
      }
    });

    // 取消按钮
    const closeBtn = this.add.rectangle(0, panelH / 2 - 24, 100, 28, 0x444444, 0.9)
      .setStrokeStyle(1, 0xaaaaaa).setInteractive({ useHandCursor: true });
    container.add(closeBtn);
    container.add(this.add.text(0, panelH / 2 - 24, '取消', {
      fontSize: '14px', color: '#fff', fontFamily: 'm6x11, pixel-zh, Microsoft YaHei, system-ui',
    }).setOrigin(0.5));
    closeBtn.on('pointerdown', () => this.closeBenchPicker());
  }

  private closeBenchPicker() {
    if (this.benchPickerUI) { this.benchPickerUI.destroy(); this.benchPickerUI = undefined; }
  }
