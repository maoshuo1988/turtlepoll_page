...
export class SceneTurtleDom {
  ...
  private applyPosition(canvasX: number, canvasY: number) {
    ...
    const sx = rect.width / game.scale.gameSize.width;
    const sy = rect.height / game.scale.gameSize.height;
    const screenX = rect.left + canvasX * sx;
    ...
    this.root.style.top = `${screenY - naturalHeight}px`;
