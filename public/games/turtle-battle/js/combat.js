...
async function doDamage(attacker, target, skill) {
  ...
  for (let i = 0; i < hits; i++) {
    ...
    let ySlot = 0;
    if (bubbleAbs > 0) { spawnFloatingNum(tElId, `-${bubbleAbs}<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'shield-dmg', 0, ySlot, { atkSide: attacker.side, amount: bubbleAbs }); ySlot += 18; }
...
async function triggerOnHitEffects(attacker, target, dmg) {
  ...
  if (attacker.passive && attacker.passive.type === 'crystalResonance' && target.alive) {
    ...
    if (target._crystallize >= maxStacks) {
      ...
      spawnFloatingNum(tElCryst, '<img src="/games/turtle-battle/assets/passive/crystal-resonance-icon.png" style="width:16px;height:16px;vertical-align:middle">引爆!', 'crit-label', 400, -30);
      recalcStats();
  ...
  if (attacker.passive && attacker.passive.type === 'lightningStorm' && target.alive) {
    ...
    if (target._shockStacks >= attacker.passive.stackMax) {
      ...
      spawnFloatingNum(tElId, `<img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">${sDmg}`, 'pierce-dmg', 300, 0);
    }
  }
...
async function tryGamblerMultiHit(attacker, target, tElId) {
  ...
  while (target.alive && attacker.alive && Math.random() * 100 < multiChance) {
    ...
    const hitIcon = '<img src="/games/turtle-battle/assets/battle/gambler-hit-icon.png" style="width:16px;height:16px;vertical-align:middle">';
    const critIcon = isCrit ? '<img src="/games/turtle-battle/assets/stats/crit-icon.png" style="width:14px;height:14px;vertical-align:middle">' : '';
...
function applyRawDmg(source, target, amount, isPierce, _skipLink, dmgType) {
  ...
  if (target._undeadLockTurns > 0) {
    ...
    if (hpBefore - rem2 <= 0) {
      const tElId = getFighterElId(target);
      spawnFloatingNum(tElId, '<img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:16px;height:16px;vertical-align:middle">无法死亡', 'crit-label', 0, -25);
    }
  ...
  if (target.hp <= 0 && target.passive && target.passive.type === 'undeadRage' && !target._undeadLockUsed) {
    ...
    spawnFloatingNum(elId, '<img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:16px;height:16px;vertical-align:middle">亡灵之力!', 'crit-label', 0, -30);
    addLog(`${target.emoji}${target.name} <span class="log-passive"><img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:16px;height:16px;vertical-align:middle">亡灵之力！锁血1HP 2回合！</span>`);
    renderStatusIcons(target);
  } else {
