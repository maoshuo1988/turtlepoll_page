...
function startMode(mode) {
  ...
  if (mode === 'pve') {
    ...
    selecting = 'left';
    selectedIds = [];
    showSelectScreen('<img src="/games/turtle-battle/assets/equip/equip-crown-icon.png" style="width:24px;height:24px;vertical-align:middle"> Boss挑战 — 选择你的队伍（选3只龟）');
...
function renderPetGrid() {
  ...
  grid.innerHTML = pets.map(p => {
    ...
    if (p.passive) {
      ...
      const iconH = iconRaw.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${iconRaw}" class="stat-icon">` : iconRaw;
    ...
    return `<div class="pet-card ${selectedIds.includes(p.id)?'selected':''}"
      ...
      <div class="pet-stats-mini">
        <span><img src="/games/turtle-battle/assets/stats/hp-icon.png" class="stat-icon">${Math.round(p.hp * getLevelBonus(p.id))}</span>
        <span><img src="/games/turtle-battle/assets/stats/atk-icon.png" class="stat-icon">${Math.round(p.atk * getLevelBonus(p.id))}</span>
        <span><img src="/games/turtle-battle/assets/stats/def-icon.png" class="stat-icon">${Math.round(p.def * getLevelBonus(p.id))}</span>
        <span><img src="/games/turtle-battle/assets/stats/mr-icon.png" class="stat-icon">${Math.round((p.mr !== undefined ? p.mr : p.def) * getLevelBonus(p.id))}</span>
      </div>
    </div>`;
  }).join('');
}
...
function startBattle(seed) {
  ...
  let bgFile = '/games/turtle-battle/assets/bg/bg-cave-alt.png';
  if (gameMode === 'boss') bgFile = '/games/turtle-battle/assets/bg/bg-cave.png';
  else if (gameMode === 'pvp-online') bgFile = '/games/turtle-battle/assets/bg/bg-shipwreck.png';
  else if (gameMode === 'dungeon') bgFile = dungeonState.stage >= 5 ? '/games/turtle-battle/assets/bg/bg-cave.png' : '/games/turtle-battle/assets/bg/bg-cave-alt.png';
  ...
  else if (gameMode === 'boss') { ll.textContent = '我方'; lr.innerHTML = '<img src="/games/turtle-battle/assets/equip/equip-crown-icon.png" style="width:20px;height:20px;vertical-align:middle"> BOSS'; }
  ...
  if (pirates.length) {
    setTimeout(async () => {
      for (const f of pirates) {
        ...
        spawnFloatingNum(fElId, '<img src="/games/turtle-battle/assets/passive/pirate-plunder-icon.png" style="width:24px;height:24px;vertical-align:middle">掠夺！', 'debuff-label', 0, -10);
        await sleep(1000);
        const enemies = (f.side === 'left' ? rightTeam : leftTeam).filter(e => e.alive);
...
function showCodexDetail(petId) {
  ...
  const statsHtml = `
    <div class="codex-stats">
      <span><img src="/games/turtle-battle/assets/stats/hp-icon.png" class="stat-icon">${Math.round(p.hp * _lb)}</span>
      <span><img src="/games/turtle-battle/assets/stats/atk-icon.png" class="stat-icon">${Math.round(p.atk * _lb)}</span>
      <span><img src="/games/turtle-battle/assets/stats/def-icon.png" class="stat-icon">${Math.round(p.def * _lb)}</span>
      <span><img src="/games/turtle-battle/assets/stats/mr-icon.png" class="stat-icon">${Math.round((p.mr !== undefined ? p.mr : p.def) * _lb)}</span>
      <span><img src="/games/turtle-battle/assets/stats/crit-icon.png" class="stat-icon">${Math.round((p.crit||0.25)*100)}%</span>
    </div>`;
  ...
  if (p.passive) {
    const iconRaw = PASSIVE_ICONS[p.passive.type] || '⭐';
    const iconH = iconRaw.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${iconRaw}" style="width:20px;height:20px;vertical-align:middle">` : iconRaw;
