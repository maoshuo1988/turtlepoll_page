...
async function doGamblerDraw(caster, _skill) {
  ...
  if (roll === 0) {
    ...
    spawnFloatingNum(fElId, `<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">回复牌`, 'passive-num', 0, -20);
    ...
    addLog(`${caster.emoji}${caster.name} <b>抽牌</b>：<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">回复牌！<span class="log-heal">+${actual}HP</span> <span class="log-shield">+${shieldAmt}护盾</span>`);
  } else if (roll === 1) {
    // 2: Bomb card — 0.9ATK to all enemies, triggers multi-hit
    spawnFloatingNum(fElId, `<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">炸弹牌`, 'crit-label', 0, -20);
    ...
    addLog(`${caster.emoji}${caster.name} <b>抽牌</b>：<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">炸弹牌！对全体敌方 <span class="log-direct">${baseDmg}伤害</span>`);
  } else {
    ...
    spawnFloatingNum(fElId, `<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">强化牌`, 'crit-label', 0, -20);
    spawnFloatingNum(fElId, `+ATK+暴击+爆伤+转真实`, 'passive-num', 200, 0);
    recalcStats();
    renderStatusIcons(caster);
    updateFighterStats(caster, fElId);
    addLog(`${caster.emoji}${caster.name} <b>抽牌</b>：<img src="/games/turtle-battle/assets/battle/card-draw-icon.png" style="width:16px;height:16px;vertical-align:middle">强化牌！<span class="log-passive">+15%ATK +25%暴击 +15%爆伤 20%伤害转真实 3回合</span>`);
  }
  await sleep(1000);
}
...
async function doFortuneDice(caster, skill) {
  ...
  const fElId = getFighterElId(caster);
  spawnFloatingNum(fElId, `<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:16px;height:16px;vertical-align:middle">${roll} +${roll}<img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'passive-num', 0, 0);
  renderStatusIcons(caster);
  ...
  updateHpBar(caster, fElId);
  addLog(`${caster.emoji}${caster.name} <b>骰子</b>：<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:16px;height:16px;vertical-align:middle">${roll}！<span class="log-passive">+${roll}金币（共${caster._goldCoins}）</span> <span class="log-heal">+${actual}HP</span>${shieldStr}`);
  await sleep(1000);
}
...
async function fireStarPassive(f, target) {
  ...
  const tElId = getFighterElId(target);
  spawnFloatingNum(tElId, `-${fireDmg}<img src="/games/turtle-battle/assets/passive/star-energy-bar-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'true-dmg', 200, 0, {atkSide:f.side, amount:fireDmg});
  updateHpBar(target, tElId);
...
async function starMeteorBurst(f) {
  ...
  for (const e of enemies) {
    ...
    spawnFloatingNum(eElId, `-${finalDmg}<img src="/games/turtle-battle/assets/passive/star-energy-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'crit-true', 300, 0, {atkSide:f.side, amount:finalDmg});
    updateHpBar(e, eElId);
  }
  renderStatusIcons(f);
  addLog(`${f.emoji}${f.name} <span class="log-passive"><img src="/games/turtle-battle/assets/passive/star-energy-icon.png" style="width:16px;height:16px;vertical-align:middle">星能爆发！</span>全体敌方 <span class="log-pierce">${burstDmg}真实伤害</span>`);
...
async function doCyberDeploy(caster, _skill) {
  ...
  const elId = getFighterElId(caster);
  spawnFloatingNum(elId, `+<img src="/games/turtle-battle/assets/passive/cyber-drone-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'passive-num', 0, 0);
  renderStatusIcons(caster);
...
async function doPirateCannonBarrage(attacker, skill) {
  ...
  for (const enemy of enemies) {
    ...
    const eEl = document.getElementById(eElId);
    if (eEl) eEl.classList.add('hit-shake');
    spawnFloatingNum(eElId, '<img src="/games/turtle-battle/assets/passive/pirate-plunder-icon.png" style="width:16px;height:16px;vertical-align:middle">炮击!', 'debuff-label', 0, -10);
  }
...
async function doPhoenixShield(caster, skill) {
  ...
  const fElId = getFighterElId(caster);
  spawnFloatingNum(fElId, `+${amount}<img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'passive-num', 0, 0);
  updateHpBar(caster, fElId);
  renderStatusIcons(caster);
...
async function doPhoenixScald(attacker, target, skill) {
  ...
  if (skill.shieldBreak && (target.shield > 0 || target.bubbleShieldVal > 0)) {
    ...
    if (target.bubbleShieldVal > 0) {
      ...
      spawnFloatingNum(tElId, `-${broken}<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'shield-dmg', 0, -15);
    }
    if (target.shield > 0) {
...
async function doBubbleShield(caster, target, skill) {
  ...
  const tElId = getFighterElId(target);
  spawnFloatingNum(tElId, `+${amount}<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'bubble-num', 0, 0);
  updateHpBar(target, tElId);
  renderStatusIcons(target);
...
async function doBubbleBind(caster, target, skill) {
  ...
  const tElId = getFighterElId(target);
  spawnFloatingNum(tElId, '<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">束缚', 'bubble-num', 0, 0);
  renderStatusIcons(target);
...
async function doGhostStorm(attacker, target, skill) {
  ...
  if (target.alive) {
    ...
    spawnFloatingNum(tElId, '<img src="/games/turtle-battle/assets/status/curse-debuff-icon.png" style="width:16px;height:16px;vertical-align:middle">诅咒', 'debuff-label', 200, -10);
    renderStatusIcons(target);
  }
...
async function doBambooChargeAttack(attacker, target) {
  ...
  // ── 蓄力停顿 ──
  spawnFloatingNum(fElId, '<img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">蓄力...', 'passive-num', 0, -20);
  ...
  spawnFloatingNum(tElId, '<img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">充能!', 'crit-label', 0, -20);
...
async function doDiceAllIn(attacker, skill) {
  ...
  const dmgType = skill.dmgType || 'physical';
  let totalDmg = 0, totalCrits = 0;
  spawnFloatingNum(fElId, '<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:16px;height:16px;vertical-align:middle">孤注一掷!', 'crit-label', 0, -20);
  for (const e of enemies) {
    if (!e.alive) continue;
    const {isCrit, critMult} = calcCrit(attacker);
    if (isCrit) totalCrits++;
...
async function doDiceFate(caster, skill) {
  ...
  recalcStats();
  spawnFloatingNum(fElId, `<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:16px;height:16px;vertical-align:middle">+${critGain}%暴击!`, 'crit-label', 0, -20);
  renderStatusIcons(caster);
  updateFighterStats(caster, fElId);
