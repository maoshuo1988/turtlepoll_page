  ...
  allFighters.forEach(f => {
    if (f.hp <= 0 && !f._deathProcessed) {
      ...
      if (f._ruleRevive) {
        ...
        const card = document.getElementById(elId);
        if (card) { card._pendingDead = false; card.classList.remove('dead','death-anim'); }
        spawnFloatingNum(elId, '<img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:16px;height:16px;vertical-align:middle">亡灵复活!', 'crit-label', 0, -25);
        updateHpBar(f, elId);
        addLog(`${f.emoji}${f.name} <span class="log-passive"><img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:16px;height:16px;vertical-align:middle">亡灵之日！以15%HP复活！</span>`);
        return;
      }
      ...
      if (f.passive && f.passive.type === 'ghostCurse') {
        ...
        for (const e of enemies) {
          ...
          const eElId = getFighterElId(e);
          spawnFloatingNum(eElId, `<img src="/games/turtle-battle/assets/status/curse-debuff-icon.png" style="width:16px;height:16px;vertical-align:middle">诅咒!`, 'crit-label', 0, -20);
          renderStatusIcons(e);
        }
      ...
      allFighters.forEach(fg => {
        if (fg.alive && fg.passive && fg.passive.type === 'fortuneGold') {
          ...
          spawnFloatingNum(fgElId, `+9<img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'passive-num', 500, 0);
...
async function processLavaTransform() {
  for (const f of allFighters) {
    ...
    if (!f._lavaTransformed && !f._lavaSpent && f._lavaRage >= f.passive.rageMax) {
      ...
      f.img = '/games/turtle-battle/assets/passive/volcano-form-icon.png';
      f.emoji = '<img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">🐢';
      f.sprite = null;
      const elId = getFighterElId(f);
      // Visual
      spawnFloatingNum(elId, '<img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">变身！', 'crit-label', 0, -30);
      ...
      renderStatusIcons(f);
      addLog(`${f.emoji}${f.name} <span class="log-passive"><img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">怒气爆发！变身为火山龟！+${hpGain}HP +${atkGain}攻 +${defGain}甲 +${mrGain}抗</span>`);
      await sleep(800);
      ...
      for (const e of enemies) {
        ...
        const eElId = getFighterElId(e);
        spawnFloatingNum(eElId, `-${dmg}<img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'magic-dmg', 0, 0, {atkSide:f.side, amount:dmg});
        updateHpBar(e, eElId);
...
function processLavaCountdown(f) {
  ...
  if (f._lavaTransformTurns <= 0) {
    ...
    f.name = '熔岩龟';
    f.img = f._lavaSmallImg || '//games/turtle-battle/assets/pets/熔岩龟.png';
    f.emoji = '<img src="/games/turtle-battle/assets/battle/lava-shield-icon.png" style="width:16px;height:16px;vertical-align:middle">🐢';
    const pet = ALL_PETS.find(p => p.id === f.id);
    ...
    for (const e of enemies) {
      if (e.hp / e.maxHp < f.passive.hpThresh / 100 && !e._undeadLockTurns) {
        ...
        try {
          ...
          icon.src = '/games/turtle-battle/assets/passive/hunter-kill-icon.png';
        ...
        spawnFloatingNum(eElId, '<img src="/games/turtle-battle/assets/passive/hunter-kill-icon.png" style="width:24px;height:24px;vertical-align:middle">猎杀!', 'crit-label', 0, -20);
...
async function processFortuneGold() {
  for (const f of allFighters) {
    ...
    spawnFloatingNum(fElId, `+${roll}<img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle">`, 'passive-num', 0, 0);
    renderStatusIcons(f);
...
async function processLightningStorm() {
  for (const f of allFighters) {
    ...
    spawnFloatingNum(eElId, `<img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">${shockDmg}`, 'pierce-dmg', 0, 0);
    updateHpBar(target, eElId);
    addLog(`${f.emoji}${f.name} 被动：<span class="log-pierce"><img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">电击${target.emoji}${target.name} ${shockDmg}真实</span>`);
...
function checkChestEquipDraw(f) {
  ...
  while (f._chestTier < thresholds.length && f._chestTreasure >= thresholds[f._chestTier]) {
    ...
    // Visual feedback
    const elId = getFighterElId(f);
    const iconH = drawn.icon.endsWith && drawn.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${drawn.icon}" style="width:16px;height:16px;vertical-align:middle">` : drawn.icon;
