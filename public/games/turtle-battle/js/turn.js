...
async function beginTurn() {
  ...
  for (const f of allFighters) {
    ...
    if (f.passive && f.passive.type === 'cyberDrone' && !f._isMech) {
      ...
      if (spawned > 0) {
        const elId = getFighterElId(f);
        spawnFloatingNum(elId, `+${spawned}<img src="/games/turtle-battle/assets/passive/cyber-drone-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'passive-num', 0, 0);
    ...
    if (f.passive.type === 'auraAwaken' && !f._auraAwakened && turnNum >= f.passive.awakenTurn) {
      ...
      // Visual + log
      spawnFloatingNum(elId, '<img src="/games/turtle-battle/assets/passive/aura-awaken-icon.png" style="width:16px;height:16px;vertical-align:middle">气场觉醒!', 'crit-label', 0, -20);
      recalcStats();
      spawnFloatingNum(elId, `+${atkGain}攻 +${defGain}护甲 +${hpGain}HP`, 'passive-num', 0, 10);
      updateHpBar(f, elId);
      updateFighterStats(f, elId);
      addLog(`${f.emoji}${f.name} <span class="log-passive"><img src="/games/turtle-battle/assets/passive/aura-awaken-icon.png" style="width:16px;height:16px;vertical-align:middle">气场觉醒！ATK+${atkGain} DEF+${defGain} HP+${hpGain} 生命偷取${f.passive.lifestealPct}% 反伤${f.passive.reflectPct}% ${f.passive.armorPenPct}%穿甲</span>`);
    }
    ...
    if (f.passive.type === 'bambooCharge') {
      ...
      if (!f._bambooCharged) {
        ...
        if (f._bambooCounter >= 2) {
          ...
          f._bambooCounter = 0;
          spawnFloatingNum(getFighterElId(f), '<img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">充能!', 'passive-num', 0, 0);
          addLog(`${f.emoji}${f.name} 被动：<span class="log-passive"><img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">竹编充能！本回合技能后追加强化攻击</span>`);
        }
      }
    ...
    if (f._pirateShipEnabled && !f._pirateShipSummoned && turnNum === 3) {
      ...
      const ship = {
        ...
        img:'/games/turtle-battle/assets/battle/pirate-ship.png', sprite:null,
  ...
  for (const f of allFighters) {
    ...
    if (p.type === 'lightningStorm') {
      ...
      if (enemies.length) {
        ...
        spawnFloatingNum(tElId, `<img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">${sDmg}`, 'pierce-dmg', 0, 0);
        updateHpBar(t, tElId);
        addLog(`${s.emoji}${s.name}(随从) 被动：<span class="log-passive"><img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">电击${t.emoji}${t.name} ${sDmg}真实</span>`);
      }
    }
    // Bamboo charge
    if (p.type === 'bambooCharge') {
      ...
      if (!s._bambooCharged) {
        ...
        if (s._bambooCounter >= 2) {
          ...
          spawnFloatingNum(sElId, '<img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">充能!', 'passive-num', 0, 0);
          addLog(`${s.emoji}${s.name}(随从) 被动：<span class="log-passive"><img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:16px;height:16px;vertical-align:middle">竹编充能！</span>`);
        }
      }
    }
    // Candy steal
...
async function processBuffs() {
  ...
  for (const f of allFighters) {
    ...
    if (f.passive && f.passive.type === 'bubbleStore' && f.bubbleStore > 0) {
      ...
      if (actual > 0) {
        spawnFloatingNum(elId, `+${actual}<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'bubble-num', 100, 0);
        updateHpBar(f, elId);
        hadTick = true;
      }
      ...
      if (f.passive.dmgPct) {
        ...
        if (dmgAmt > 0) {
          ...
          if (enemies.length) {
            ...
            const tElId = getFighterElId(target);
            spawnFloatingNum(tElId, `-${finalDmg}<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:16px;height:16px;vertical-align:middle">`, 'magic-dmg', 100, 0, { atkSide: f.side, amount: finalDmg });
            updateHpBar(target, tElId);
            hadTick = true;
          }
        }
