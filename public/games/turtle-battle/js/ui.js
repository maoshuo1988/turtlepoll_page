...
function renderSceneBuffs(f) {
  ...
  if (!box) return;
  const ic = (src) => `<img src="/games/turtle-battle/assets/${src}" style="width:14px;height:14px;vertical-align:middle">`;
  const icons = [];
  ...
  if (f._equips && f._equips.length) {
    for (const eq of f._equips) {
      const eIcon = eq.icon && eq.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${eq.icon}" style="width:12px;height:12px" title="${eq.name}">` : (eq.icon||'');
...
function refreshDetailPanel(f) {
  ...
  const ic = (name) => `<img src="/games/turtle-battle/assets/stats/${name}" class="stat-icon">`;
...
function showFighterDetail(f) {
  ...
  if (f.buffs && f.buffs.length) {
    ...
    f.buffs.forEach(b => {
      if (b.type === 'phoenixBurnDot') html += tag('#ff6600', `<img src="/games/turtle-battle/assets/status/burn-icon.png" style="width:14px;height:14px;vertical-align:middle">灼烧 ${b.turns}回合`);
      else if (b.type === 'dot') html += tag('#9b59b6', `<img src="/games/turtle-battle/assets/status/curse-debuff-icon.png" style="width:14px;height:14px;vertical-align:middle">诅咒 ${b.turns}回合`);
      ...
      else if (b.type === 'dodge') html += tag('#aaa', `<img src="/games/turtle-battle/assets/status/dodge-new-icon.png" style="width:14px;height:14px;vertical-align:middle">闪避${b.value}% ${b.turns}回合`);
      else if (b.type === 'stun') html += tag('#ffee00', `<img src="/games/turtle-battle/assets/status/stun-icon.png" style="width:14px;height:14px;vertical-align:middle">眩晕`);
      else if (b.type === 'healReduce') html += tag('#6b8e23', `<img src="/games/turtle-battle/assets/status/heal-reduce-icon.png" style="width:14px;height:14px;vertical-align:middle">治疗削减 ${b.value}%`);
      else if (b.type === 'hot') html += tag('#06d6a0', `持续回复 ${b.hpPerTurn}/回 ${b.turns}回合`);
      else if (b.type === 'fear') html += tag('#9b59b6', `<img src="/games/turtle-battle/assets/status/fear-icon.png" style="width:14px;height:14px;vertical-align:middle">恐惧 ${b.turns}回合`);
      else if (b.type === 'bubbleBind') html += tag('#4cc9f0', `<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">泡泡束缚 ${b.turns}回合`);
      else if (b.type === 'trap') html += tag('#ff9f43', `<img src="/games/turtle-battle/assets/passive/ninja-instinct-icon.png" style="width:14px;height:14px;vertical-align:middle">陷阱`);
      else if (b.type === 'diceFateCrit') html += tag('#ff4757', `<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle">暴击+${b.value}% ${b.turns}回合`);
      else if (b.type === 'gamblerPierceConvert') html += tag('#ffd93d', `<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle">穿透转换 ${b.turns}回合`);
      else if (b.type === 'hidingShield') html += tag('#fff', `<img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">缩头护盾 ${b.turns}回合`);
      else if (b.type === 'poison') html += tag('#6b8e23', `<img src="/games/turtle-battle/assets/status/poison-icon.png" style="width:14px;height:14px;vertical-align:middle">中毒 ${b.value}/回 ${b.turns}回合`);
      else if (b.type === 'chilled') html += tag('#87ceeb', `<img src="/games/turtle-battle/assets/status/chilled-icon.png" style="width:14px;height:14px;vertical-align:middle">冰寒 ATK-20% ${b.turns}回合`);
      else if (b.type === 'bleed') html += tag('#cc3333', `<img src="/games/turtle-battle/assets/status/bleed-icon.png" style="width:14px;height:14px;vertical-align:middle">流血 ${b.value}/回 ${b.turns}回合`);
    });
    // Non-buff state tags
    if (f._inkStacks > 0) html += tag('#222', `<img src="/games/turtle-battle/assets/passive/ink-mark-icon.png" style="width:14px;height:14px;vertical-align:middle">墨迹 ${f._inkStacks}层 (受伤+${f._inkStacks * 5}%)`);
    if (f._shockStacks > 0) html += tag('#ffd700', `<img src="/games/turtle-battle/assets/passive/lightning-storm-icon.png" style="width:14px;height:14px;vertical-align:middle">电击 ${f._shockStacks}层`);
    if (f._goldLightning > 0) html += tag('#ffd700', `⚡金闪电 ${f._goldLightning}/8`);
    html += '</div>';
  } else {
    // No buffs but might have special states
    const specials = [];
    if (f._inkStacks > 0) specials.push(`<span class="fdp-buff-tag" style="border-color:#b8b8ff;color:#b8b8ff;background:rgba(100,100,200,.2)"><img src="/games/turtle-battle/assets/passive/ink-mark-icon.png" style="width:14px;height:14px;vertical-align:middle">墨迹 ${f._inkStacks}层</span>`);
  ...
  if (f.passive) {
    ...
    const iconH = iconRaw.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${iconRaw}" style="width:16px;height:16px;vertical-align:middle">` : iconRaw;
    ...
    if (f.passive.type === 'chestTreasure') {
      ...
      const renderPool = (label, pool) => {
        ...
        for (const eq of pool) {
          ...
          const eIcon = eq.icon && eq.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${eq.icon}" style="width:14px;height:14px;vertical-align:middle;${isOwned?'':'opacity:.5'}">` : (eq.icon||'');
    ...
    if (f.passive.type === 'cyberDrone') st.push(`<img src="/games/turtle-battle/assets/passive/cyber-drone-icon.png" style="width:14px;height:14px;vertical-align:middle"> 浮游炮：<span class="val-atk">${f._drones ? f._drones.length : 0}</span> / ${f.passive.maxDrones}`);
    ...
    if (f.passive.type === 'inkMark') st.push(`<img src="/games/turtle-battle/assets/passive/ink-mark-icon.png" style="width:14px;height:14px;vertical-align:middle"> 墨迹层数：<span class="val-atk">${f._inkStacks||0}</span> / ${f.passive.maxStacks}`);
    if (f.passive.type === 'lightningStorm') st.push(`⚡ 全局电击层：<span class="val-atk">${f._shockStacks||0}</span> / ${f.passive.stackMax}`);
    if (f.passive.type === 'gamblerBlood') { const oc = Math.max(0, (f.crit||0) - 1.0); st.push(`<img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle"> 暴击溢出：<span class="val-atk">${oc > 0 ? Math.round(oc*100)+'%→+'+Math.round(oc*f.passive.overflowMult*100)+'%爆伤' : '无'}</span>`); }
  ...
  // ── Equipment ──
  if (f._equips && f._equips.length) {
    html += `<div class="fdp-equips"><b>装备：</b>${f._equips.map(e => (e.icon.endsWith && e.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${e.icon}" style="width:14px;height:14px;vertical-align:middle">` : e.icon) + e.name).join('、')}</div>`;
  }
...
function renderSummonStatusIcons(summon) {
  ...
  const buffHTML = (summon.buffs || []).map(b => {
    if (b.type === 'dot')     return `<span class="status-dot" title="诅咒${b.value}/回合 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/curse-debuff-icon.png" style="width:12px;height:12px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'phoenixBurnDot') return `<span class="status-dot" title="灼烧 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/burn-icon.png" style="width:12px;height:12px;vertical-align:middle">${b.turns}</span>`;
    ...
    if (b.type === 'bubbleBind') return `<span class="status-bubble"><img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:12px;height:12px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'dodge')   return `<span class="status-dodge"><img src="/games/turtle-battle/assets/status/dodge-icon.png" style="width:12px;height:12px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'fear')    return `<span class="status-atkdown"><img src="/games/turtle-battle/assets/status/fear-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'wormhole') return `<span style="color:#ffa500"><img src="/games/turtle-battle/assets/status/wormhole-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'gamblerPierceConvert') return `<span class="status-defup">🗡${b.turns}</span>`;
    if (b.type === 'hidingShield') return `<span class="status-defup"><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:12px;height:12px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'stun')    return `<span style="color:#ff0">💫</span>`;
    if (b.type === 'diceFateCrit') return `<span style="color:#ff6b6b"><img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle">+${b.value}%</span>`;
  ...
  if (summon._inkStacks > 0) {
    box.innerHTML += `<span style="color:#1a1a2e;background:rgba(100,100,100,.2);padding:0 3px;border-radius:4px"><img src="/games/turtle-battle/assets/passive/ink-mark-icon.png" style="width:12px;height:12px;vertical-align:middle">${summon._inkStacks}</span>`;
  }
  // Ink link
  ...
  if (summon._starEnergy > 0) {
    box.innerHTML += `<span style="color:#ffa500"><img src="/games/turtle-battle/assets/passive/star-energy-bar-icon.png" style="width:12px;height:12px;vertical-align:middle">${Math.round(summon._starEnergy)}</span>`;
  }
  // Gold coins
  if (summon._goldCoins > 0) {
    box.innerHTML += `<span style="color:#ffd93d"><img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:12px;height:12px;vertical-align:middle">${summon._goldCoins}</span>`;
  }
  // Drone count
  if (summon._drones && summon._drones.length > 0) {
    box.innerHTML += `<span style="color:#4cc9f0"><img src="/games/turtle-battle/assets/passive/cyber-drone-icon.png" style="width:12px;height:12px;vertical-align:middle">${summon._drones.length}</span>`;
  }
  // Bamboo charge
  if (summon._bambooCharged && !summon._bambooFired) {
    box.innerHTML += `<span class="bamboo-charge-ready"><img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" style="width:12px;height:12px;vertical-align:middle"></span>`;
  }
  // Lava rage
  if (summon._lavaRage > 0 && !summon._lavaTransformed) {
    box.innerHTML += `<span style="color:#ff6600"><img src="/games/turtle-battle/assets/passive/lava-heart-icon.png" style="width:12px;height:12px;vertical-align:middle">${summon._lavaRage}</span>`;
  }
}
...
function updateFighterStats(f, elId) {
  ...
  const _piHtml = _pi.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${_pi}" class="stat-icon">` : _pi;
...
function buildPetAvatarHTML(pet, size) {
  return `<img src="/games/turtle-battle/assets/avatars/${pet.id}.png" style="width:${size}px;height:${size}px;object-fit:cover;border-radius:50%" onerror="this.style.display='none'">`;
}
...
function updateHpBar(f, elId) {
  ...
  let hpLine = `<div class="hp-line"><img src="/games/turtle-battle/assets/stats/hp-icon.png" class="stat-icon"> ${Math.ceil(f.hp)}/<span class="${maxHpClass}">${f.maxHp}</span></div>`;
  const shieldParts = [];
  if (f.shield > 0) shieldParts.push(`<span class="shield-val"><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">${Math.ceil(f.shield)}</span>`);
  if (f.bubbleShieldVal > 0) shieldParts.push(`<span class="bubble-val"><img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">${Math.ceil(f.bubbleShieldVal)} <small>${f.bubbleShieldTurns}回合</small></span>`);
...
function renderStatusIcons(f) {
  ...
  box.innerHTML = f.buffs.map(b => {
    if (b.type === 'dot')     return `<span class="status-dot" title="诅咒${b.value}/回合 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/curse-debuff-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'phoenixBurnDot') return `<span class="status-dot" title="灼烧(${b.value}+${b.hpPct}%HP)/回合 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/burn-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    ...
    if (b.type === 'bubbleBind') return `<span class="status-bubble" title="被束缚：攻击者获得${b.value}%伤害护盾 剩${b.turns}回合"><img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'dodge') return `<span class="status-dodge" title="闪避${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/dodge-new-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'fear')  return `<span class="status-atkdown" title="恐惧：对双头龟伤害-${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/fear-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'wormhole') return `<span style="color:#ffa500;background:rgba(255,165,0,.15);padding:1px 5px;border-radius:6px" title="虫洞标记：真实+${b.pierceBonusPct}% 魔伤+${b.normalBonusPct}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/wormhole-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'gamblerPierceConvert') return `<span class="status-defup" title="${b.value}%伤害转真实 剩${b.turns}回合">🗡${b.turns}</span>`;
    if (b.type === 'hidingShield') return `<span class="status-defup" title="缩头护盾 剩${b.turns}回合，到期回复剩余盾${b.healPct}%HP"><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'stun') return `<span style="color:#ff0;background:rgba(255,255,0,.2);padding:1px 5px;border-radius:6px" title="眩晕：跳过下次行动"><img src="/games/turtle-battle/assets/status/stun-icon.png" style="width:14px;height:14px;vertical-align:middle">眩晕</span>`;
    if (b.type === 'diceFateCrit') return `<span style="color:#ff6b6b;background:rgba(255,107,107,.15);padding:1px 5px;border-radius:6px" title="命运骰子+${b.value}%暴击 剩${b.turns}回合"><img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle">+${b.value}%</span>`;
    if (b.type === 'healReduce') return `<span style="color:#6b8e23;background:rgba(107,142,35,.15);padding:1px 5px;border-radius:6px" title="治疗削减-${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/heal-reduce-icon.png" style="width:14px;height:14px;vertical-align:middle">-${b.value}%治疗${b.turns}</span>`;
    if (b.type === 'taunt') return `<span style="color:#ff4444;background:rgba(255,68,68,.15);padding:1px 5px;border-radius:6px" title="嘲讽 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/taunt-icon.png" style="width:14px;height:14px;vertical-align:middle">嘲讽${b.turns}</span>`;
    if (b.type === 'reflect') return `<span style="color:#ff8c00;background:rgba(255,140,0,.15);padding:1px 5px;border-radius:6px" title="反弹${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/reflect-icon.png" style="width:14px;height:14px;vertical-align:middle">反弹${b.turns}</span>`;
    if (b.type === 'dmgReduce') return `<span style="color:#4dabf7;background:rgba(77,171,247,.15);padding:1px 5px;border-radius:6px" title="受伤-${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">-${b.value}%${b.turns}</span>`;
    if (b.type === 'dodgeCounter') return `<span style="color:#ffa500;background:rgba(255,165,0,.15);padding:1px 5px;border-radius:6px" title="闪避反击${b.value}伤害 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/counter-icon.png" style="width:14px;height:14px;vertical-align:middle">反击${b.turns}</span>`;
    if (b.type === 'lifesteal') return `<span style="color:#e74c3c;background:rgba(231,76,60,.15);padding:1px 5px;border-radius:6px" title="吸血+${b.value}% 剩${b.turns}回合"><img src="/games/turtle-battle/assets/stats/lifesteal-icon.png" style="width:14px;height:14px;vertical-align:middle">+${b.value}%${b.turns}</span>`;
    ...
    if (b.type === 'poison') return `<span style="color:#6b8e23;background:rgba(107,142,35,.15);padding:1px 5px;border-radius:6px" title="中毒${b.value}/回合 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/poison-icon.png" style="width:14px;height:14px;vertical-align:middle">中毒${b.turns}</span>`;
    if (b.type === 'bleed') return `<span style="color:#cc3333;background:rgba(204,51,51,.15);padding:1px 5px;border-radius:6px" title="流血${b.value}/回合 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/bleed-icon.png" style="width:14px;height:14px;vertical-align:middle">流血${b.turns}</span>`;
    if (b.type === 'counter') return `<span style="color:#ffa500;background:rgba(255,165,0,.15);padding:1px 5px;border-radius:6px" title="反击${b.value}伤害 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/counter-icon.png" style="width:14px;height:14px;vertical-align:middle">${b.turns}</span>`;
    if (b.type === 'physImmune') return `<span style="color:#9b59b6;background:rgba(155,89,182,.15);padding:1px 5px;border-radius:6px" title="虚化：免疫物理伤害 剩${b.turns}回合"><img src="/games/turtle-battle/assets/status/stealth-icon.png" style="width:14px;height:14px;vertical-align:middle">虚化${b.turns}</span>`;
    if (b.type === 'hunterMark') return `<span style="color:#ff4444;background:rgba(255,68,68,.2);padding:1px 5px;border-radius:6px" title="猎杀印记：HP<${b.value}%时被斩杀 剩${b.turns}回合"><img src="/games/turtle-battle/assets/passive/hunter-kill-icon.png" style="width:14px;height:14px;vertical-align:middle">印记${b.turns}</span>`;
    return '';
  }).join('');
  ...
  if (f._starEnergy > 0) {
    ...
    box.innerHTML += `<span style="color:${full?'#ffd700':'#ffa500'};background:rgba(255,215,0,.15);padding:1px 5px;border-radius:6px" title="星能${Math.round(f._starEnergy)}/${maxE}${full?' 满能！下次攻击爆发！':''}"><img src="/games/turtle-battle/assets/passive/star-energy-bar-icon.png" style="width:14px;height:14px;vertical-align:middle">${Math.round(f._starEnergy)}${full?'💥':''}</span>`;
  }
  // Drone count indicator
  if (f._drones && f._drones.length > 0) {
    const oldest = Math.max(...f._drones.map(d => d.age));
    box.innerHTML += `<span class="status-defup" title="浮游炮${f._drones.length}个，最老${oldest}回合" style="color:#4cc9f0;background:rgba(76,201,240,.15)"><img src="/games/turtle-battle/assets/passive/cyber-drone-icon.png" style="width:14px;height:14px;vertical-align:middle">${f._drones.length}</span>`;
  }
  ...
  if (f._goldCoins > 0) {
    box.innerHTML += `<span class="status-defup" title="金币${f._goldCoins}" style="color:#ffd93d;background:rgba(255,217,61,.15)"><img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle">${f._goldCoins}</span>`;
  }
  ...
  if (f._bambooCharged && !f._bambooFired) {
    box.innerHTML += `<span class="bamboo-charge-ready" title="竹编充能：本回合技能后追加强化攻击"><img src="/games/turtle-battle/assets/passive/bamboo-charge-icon.png" class="stat-icon">充能</span>`;
  }
  // Crystallize stacks indicator
  if (f._crystallize > 0) {
    box.innerHTML += `<span style="color:#c77dff;background:rgba(199,125,255,.15);padding:1px 5px;border-radius:6px" title="结晶${f._crystallize}/4，满4层引爆"><img src="/games/turtle-battle/assets/passive/crystal-resonance-icon.png" style="width:14px;height:14px;vertical-align:middle">${f._crystallize}</span>`;
  }
  // Diamond collide stacks indicator
  if (f._collideStacks > 0) {
    box.innerHTML += `<span style="color:#b8d4e3;background:rgba(184,212,227,.15);padding:1px 5px;border-radius:6px" title="碰撞${f._collideStacks}/2，满2次眩晕"><img src="/games/turtle-battle/assets/passive/diamond-structure-icon.png" style="width:14px;height:14px;vertical-align:middle">${f._collideStacks}</span>`;
  }
  // Rainbow prism color indicator
  if (f._prismColor !== undefined && f.passive && f.passive.type === 'rainbowPrism') {
  ...
  if (f._undeadLockTurns > 0) {
    box.innerHTML += `<span style="color:#9b59b6;background:rgba(155,89,182,.2);padding:1px 5px;border-radius:6px;font-weight:700" title="亡灵之力：锁血1HP ${f._undeadLockTurns}回合"><img src="/games/turtle-battle/assets/passive/undead-rage-icon.png" style="width:14px;height:14px;vertical-align:middle">锁血${f._undeadLockTurns}</span>`;
  }
  // Lava rage indicator
  if (f.passive && f.passive.type === 'lavaRage') {
    if (f._lavaTransformed) {
      box.innerHTML += `<span style="color:#ff6600;background:rgba(255,102,0,.2);padding:1px 5px;border-radius:6px;font-weight:700" title="火山形态 剩余${f._lavaTransformTurns}回合"><img src="/games/turtle-battle/assets/passive/volcano-form-icon.png" style="width:14px;height:14px;vertical-align:middle">火山${f._lavaTransformTurns}</span>`;
    } else if (!f._lavaSpent) {
      const rage = f._lavaRage || 0;
      box.innerHTML += `<span style="color:#ff6600;background:rgba(255,102,0,.15);padding:1px 5px;border-radius:6px" title="怒气${rage}/100"><img src="/games/turtle-battle/assets/passive/lava-heart-icon.png" style="width:14px;height:14px;vertical-align:middle">${rage}/100</span>`;
    }
  }
  ...
  if (f.passive && f.passive.type === 'chestTreasure') {
    ...
    if (f._chestEquips && f._chestEquips.length > 0) {
      const equipIcons = f._chestEquips.map(e => {
        const ih = e.icon.endsWith && e.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${e.icon}" style="width:14px;height:14px;vertical-align:middle">` : e.icon;
...
function renderActionButtons(f) {
  ...
  box.innerHTML = f.skills.map((s,i) => {
    ...
    const shieldImg = '<img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:16px;height:16px;vertical-align:middle">';
    const iconMap = {physical:'⚔️',magic:'✨',heal:'💚',shield:shieldImg,bubbleShield:'<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:16px;height:16px;vertical-align:middle">',bubbleBind:'<img src="/games/turtle-battle/assets/passive/bubble-store-icon.png" style="width:16px;height:16px;vertical-align:middle">',hidingDefend:shieldImg,hidingCommand:'🫣'};
...
function buildSkillDetail(s, f) {
  ...
  // ── Type label ──
  const typeMap = {
    physical:'⚔️ 物理', magic:'✨ 魔法', heal:'💚 治疗', shield:'<img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle"> 护盾',
    bubbleShield:'<img src=\"/games/turtle-battle/assets/passive/bubble-store-icon.png\" style=\"width:14px;height:14px;vertical-align:middle\"> 泡泡盾', bubbleBind:'<img src=\"/games/turtle-battle/assets/passive/bubble-store-icon.png\" style=\"width:14px;height:14px;vertical-align:middle\"> 泡泡束缚',
    ...
    fortuneDice:'<img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle"> 骰子', fortuneAllIn:'<img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle"> 梭哈',
    hidingDefend:'<img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle"> 缩头防御', hidingCommand:'🫣 指挥',
    angelBless:'😇 祝福', angelEquality:'⚖️ 平等',
    iceSpike:'❄️ 冰锥', iceFrost:'❄️ 冰霜',
    turtleShieldBash:'<img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle"> 龟盾', basicBarrage:'🐢 打击',
  };
  ...
  // ── Random ──
  if (s.random) lines.push(`<b><img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle">随机</b> 伤害×0.5~1.5倍率`);
  ...
  if (s.type === 'hunterStealth') {
    ...
    lines.push(`<b><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">护盾</b> ${s.shieldScale}×ATK = ${S(sv)}`);
  }
  ...
  if (s.type === 'fortuneDice') {
    ...
    lines.push(`<b><img src="/games/turtle-battle/assets/passive/gambler-blood-icon.png" style="width:14px;height:14px;vertical-align:middle"></b> 1~6金币 + ${H(v+'HP')}`);
  }
  if (s.type === 'fortuneAllIn') {
    const pp = f?Math.round(atk*s.perCoinAtkPierce):'?'; const np = f?Math.round(atk*s.perCoinAtkNormal):'?';
    lines.push(`<b><img src="/games/turtle-battle/assets/battle/gold-coin-icon.png" style="width:14px;height:14px;vertical-align:middle">梭哈</b> 每币 ${N(np+'普')}+${P(pp+'穿')}`);
    lines.push(`<b>⚠</b> 一场限一次`);
  }
  if (s.type === 'hidingDefend') {
    const v = f?Math.round(maxHp*s.shieldHpPct/100):'?';
    lines.push(`<b><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">护盾</b> ${S(v)} ${s.shieldDuration}回合`);
  ...
  if (s.type === 'turtleShieldBash') {
    ...
    lines.push(`<b><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">护盾</b> 获得造成伤害${s.shieldFromDmgPct}%的永久护盾`);
  }
  ...
  if (s.shieldAtkScale) {
    lines.push(`<b><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">护盾</b> ${s.shieldAtkScale}×ATK`);
  }

  // Angel turtle
  if (s.type === 'angelBless') {
    lines.push(`<b><img src="/games/turtle-battle/assets/status/shield-icon.png" style="width:14px;height:14px;vertical-align:middle">护盾</b> ${s.shieldScale}×ATK 持续${s.shieldTurns}回合`);
...
function showPassivePopup(e, fIdx) {
  ...
  const iconRaw = PASSIVE_ICONS[f.passive.type] || '⭐';
  const iconHtml = iconRaw.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${iconRaw}" class="passive-popup-icon">` : iconRaw;
  ...
  if (f.passive.type === 'chestTreasure') {
    ...
    const renderPool = (label, pool) => {
      ...
      for (const eq of ownedInPool) {
        const eIcon = eq.icon.endsWith && eq.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${eq.icon}" style="width:14px;height:14px;vertical-align:middle">` : eq.icon;
        html += `<br><span style="color:#c77dff">${eIcon} ${eq.name}：${eq.desc}</span>`;
      }
      for (const eq of unownedInPool) {
        const eIcon = eq.icon.endsWith && eq.icon.endsWith('.png') ? `<img src="/games/turtle-battle/assets/${eq.icon}" style="width:14px;height:14px;vertical-align:middle;opacity:.5">` : eq.icon;
