...
// ── TAB SWITCHING ──
const ALL_PAGES = ['all','wc','hot','tech','ai','ent','forum','battle'];
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const initialTab = ALL_PAGES.includes(tabFromUrl) ? tabFromUrl : 'all';
  ...
  const target = document.getElementById('page-' + initialTab);
  if (target) target.style.display = 'block';

  const nav = document.querySelector(`.nt[onclick*="'${initialTab}'"]`);
  if (nav) {
    document.querySelectorAll('.nt').forEach(x => x.classList.remove('on'));
...
function recordVoteResult() {
  ...
  let streak = parseInt(localStorage.getItem('turtleWinStreak') || '0');
  if (won) {
    streak++;
    localStorage.setItem('turtleWinStreak', streak.toString());
    if (streak >= 5) toast('🔥','连胜 ' + streak + ' 局！披风解锁条件达成！');
  } else {
    localStorage.setItem('turtleWinStreak', '0');
  }
}
...
function confirmVote(){if(!mChoice){toast('⚠️','请先选择你的判断');return;}closeModal();toast('🎉','押注成功！'+mAmt+' 🪙 已押「'+(mChoice==='y'?'是':'否')+'」');recordVoteResult();}
