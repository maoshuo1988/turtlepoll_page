/**
 * 文件说明：prediction Api 接口地址常量，供请求 Hook 统一引用。
 */
export {
  API_Predict_Markets as API_Football_Markets,
  API_Predict_Markets_By_Name as API_Football_Markets_By_Name,
  API_Predict_Markets_By_Tag as API_Football_Markets_By_Tag,
  API_Predict_Bet_Settle_Result as API_Football_Bet_Settle_Result,
  API_Predict_Context_Update as API_Football_Predict_Context_Update,
  API_Predict_Context_Hot as API_Football_Predict_Context_Hot,
  API_Predict_Tags_Hot as API_Football_Predict_Tags_Hot,
} from '@/api/predictApi';
