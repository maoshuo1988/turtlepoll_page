...
export const PredictionsView: React.FC<PredictionsViewProps> = ({ heroNews, items, onBet, onEnterBattle }) => {
  return (
    <section className="view-shell view-rhythm view-predictions w-full max-w-none mx-0 grid gap-4">
      <HeroPrediction news={heroNews} onBet={onBet} onEnterBattle={onEnterBattle} />
      <NewsFeed items={items} onBet={onBet} onEnterBattle={onEnterBattle} />
    </section>
  );
};
