...
export const ActivePredictionsPage: React.FC<ActivePredictionsPageProps> = ({
  ...
  return (
    <section className="mx-0 grid w-full max-w-none gap-4">
      <div className="rounded-[24px] border border-white/8 bg-[linear-gradient(180deg,#101114_0%,#0a0b0d_100%)] px-4 py-4 shadow-[0_18px_40px_rgba(0,0,0,0.28)]">
        <div className="flex items-center justify-between gap-3">
          <button
            type="button"
            onClick={onBack}
            className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-zinc-300 transition-colors hover:bg-white/[0.06] hover:text-white"
          >
            <ArrowLeft size={18} />
          </button>
          <div className="min-w-0 flex-1 text-center">
            <div className="text-[11px] font-semibold tracking-[0.22em] text-zinc-500">LIVE BATTLES</div>
            <h2 className="mt-1 text-[22px] font-black tracking-[-0.03em] text-white">进行中对局</h2>
          </div>
          <div className="inline-flex min-w-[72px] items-center justify-center rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 py-1 text-[12px] font-bold text-emerald-300">
            {items.length} 场
          </div>
        </div>
      </div>
