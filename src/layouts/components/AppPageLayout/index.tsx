...
import type { ViewType } from '@/components/pc/layout/Sidebar';
  ...
  return (
    <div className={css("legacy-fusion-app fixed-sidebar-style flex h-screen flex-col overflow-hidden bg-[#080808] text-white transition-colors dark:bg-rdark")}>
      ...
      {showSidebar && sidebarProps ? (
        <main className={css(`app-main flex min-h-0 flex-1 flex-col overflow-hidden lg:flex-row ${mainClassName}`)}>
          {/* PC 端左侧栏固定宽度，移动端导航交给 MobileFooter。 */}
          <aside className={css("app-sidebar hidden w-[276px] shrink-0 self-stretch overflow-hidden lg:flex lg:flex-col")}>
          ...
          <div
            ...
            className={`${css("app-content min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto rounded-none border-0 bg-transparent shadow-none lg:h-full lg:overflow-y-auto")} app-content`}
          >
