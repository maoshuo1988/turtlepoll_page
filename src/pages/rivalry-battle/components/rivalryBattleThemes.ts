...
export interface RivalryBattleThemeSide {
  primary: string;
  accent: string;
  deep: string;
  rgb: string;
...
export interface RivalryBattleTheme {
  id: string;
  sideA: RivalryBattleThemeSide;
  sideB: RivalryBattleThemeSide;
}

export interface RivalryBattleThemeOverrides {
  sideAImage?: string;
  sideBImage?: string;
  sideAColor?: string;
  sideBColor?: string;
...
type ThemePalette = {
  id: string;
  sideA: Omit<RivalryBattleThemeSide, 'imageUrl'> & { icon: string };
  sideB: Omit<RivalryBattleThemeSide, 'imageUrl'> & { icon: string };
};
...
export function resolveRivalryBattleTheme(
  seed: string | number,
  leftLabel: string,
  rightLabel: string,
  overrides: RivalryBattleThemeOverrides = {},
): RivalryBattleTheme {
  const normalizedSeed = String(seed || 'rivalry-battle');
