...
export function WorldCupPitchBackdropLayers() {
  return (
    <>
      <style>{`
        @keyframes wcfx-turf-drift {
          0% { background-position: 0 0; }
          100% { background-position: 144px 0; }
        }
        @keyframes wcfx-flood-sweep {
          0% { transform: translateX(-38%) skewX(-12deg); opacity: 0; }
          22% { opacity: 1; }
          100% { transform: translateX(145%) skewX(-12deg); opacity: 0; }
        }
        @keyframes wcfx-flood-soft {
          0%, 100% { opacity: 0.38; transform: scale(1); }
          50% { opacity: 0.56; transform: scale(1.07); }
        }
        @keyframes wcfx-ball-float {
          0%, 100% { transform: translate(0, 0) rotate(0deg); }
          40% { transform: translate(-5px, 4px) rotate(-10deg); }
          70% { transform: translate(5px, -3px) rotate(8deg); }
        }
        @keyframes wcfx-grass-rise {
          0% { transform: translateY(12%) scale(0.65); opacity: 0; }
          35% { opacity: 1; }
          100% { transform: translateY(-120%) scale(1); opacity: 0; }
        }
        @keyframes wcfx-center-pulse {
          0%, 100% { opacity: 0.35; }
          45% { opacity: 1; }
        }
      ...
      <div
        aria-hidden
        className="wcfx-turf-stripes pointer-events-none absolute inset-0 opacity-[0.24] mix-blend-soft-light"
        style={{
          backgroundImage:
          ...
          animation: 'wcfx-turf-drift 44s linear infinite',
        }}
      />
      <div
        aria-hidden
        className="wcfx-turf-stripes pointer-events-none absolute inset-0 opacity-[0.1] mix-blend-screen"
        style={{
          backgroundImage:
            'repeating-linear-gradient(0deg, transparent 0px, transparent 3px, rgba(52,255,139,0.14) 3px, rgba(52,255,139,0.14) 4px)',
          animation: 'wcfx-turf-drift 80s linear reverse infinite',
        }}
      />
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -left-[18%] -top-[30%] h-[125%] w-[72%] rounded-full bg-emerald-400/[0.18] blur-3xl"
        style={{ animation: 'wcfx-flood-soft 8s ease-in-out infinite' }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-[20%] -top-[32%] h-[132%] w-[68%] rounded-full bg-cyan-400/[0.16] blur-3xl"
        style={{ animation: 'wcfx-flood-soft 11s ease-in-out infinite 1.4s' }}
      />

      {/* 掠过光帘（双相位，更明显） */}
      <div
        aria-hidden
        className="wcfx-flood-sheet pointer-events-none absolute inset-y-[-12%] -left-[42%] w-[48%] bg-gradient-to-r from-transparent via-white/[0.14] to-transparent mix-blend-screen"
        style={{ animation: 'wcfx-flood-sweep 17s ease-in-out infinite' }}
      />
      <div
        aria-hidden
        className="wcfx-flood-sheet pointer-events-none absolute inset-y-[-8%] -left-[45%] w-[38%] bg-gradient-to-r from-transparent via-emerald-200/[0.09] to-transparent mix-blend-plus-lighter"
        style={{ animation: 'wcfx-flood-sweep 24s ease-in-out infinite', animationDelay: '7s' }}
      />

      {/* 中线开球圆：外环呼吸（CSS 叠加） */}
      <div
        aria-hidden
        className="wcfx-field-pulse pointer-events-none absolute inset-0 opacity-[0.22]"
        style={{
          background:
            'radial-gradient(circle at 50% 50%, rgba(52,255,139,0) calc(42% - 1px), rgba(167,243,208,0.35) 42%, rgba(167,243,208,0) 44%)',
          animation: 'wcfx-center-pulse 14s ease-in-out infinite',
        }}
      />
      ...
      {[
        ...
        <span
          key={idx}
          aria-hidden
          className="wcfx-grass-bit pointer-events-none absolute bottom-[14%]"
          style={{
            left: dot.left,
            width: dot.w,
            height: dot.w + 6,
            ...
            filter: 'blur(1px)',
            borderRadius: 999,
            animation: `wcfx-grass-rise ${dot.dur} ease-in-out infinite`,
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-0 opacity-[0.088]"
        style={{
      ...
      {/* 悬浮足球（更明显 + 大一点） */}
      <div className="pointer-events-none absolute -bottom-1 right-0 h-[5.75rem] w-[5.75rem] opacity-[0.26] blur-[1px] sm:bottom-1 sm:right-2 sm:h-36 sm:w-36">
        <div
          className="wcfx-ball-orbit absolute inset-[7px] rounded-full sm:inset-3"
          style={{
            background:
            ...
            animation: 'wcfx-ball-float 16s ease-in-out infinite',
            boxShadow:
...
export function WorldCupCardPitchTexture() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden rounded-[inherit]">
      ...
      />
      <div
        className="absolute -right-[12%] -top-[20%] h-[65%] w-[55%] rounded-full opacity-[0.11]"
        style={{
          background:
